...
case "${PROFILE}" in
  ...
  trainer-chunk-true-fused-tile16-wmma|trainer_chunk_true_fused_tile16_wmma|true-fused-trainer-chunk-tile16-wmma|true_fused_trainer_chunk_tile16_wmma)
    ...
    DEFAULT_MAX_CUBLASLT_REFERENCE_RATIO=1.000
    REJECTED_PROFILE="${PROFILE}"
    REJECTED_REASON="Production-shape focused strict true-fused LM-head tile16 WMMA profile. It builds the candidate Tile ops library with NFN_TILE_CUDA_LM_HEAD_TRUE_FUSED_MAT_TILE=16 and NFN_TILE_CUDA_LM_HEAD_TRUE_FUSED_WMMA=1, forces CE threads to 256, and measures the cooperative single-kernel CE+dHidden+dWeight body. The 2026-06-29 focused probe proved strict-true-fused-tile-kernel with candidate_symbol_abi_implementation_class=wmma-bf16-cooperative-tile-experimental, but still rejected it at 4.041717x candidate/current-wrapper, 2.850141x candidate/reference-summed, and 2.202791x candidate/reference-summed-with-logits time. The strict body took 94.806450 ms/iter. Keep rejected until this focused gate proves candidate/current-wrapper and candidate/reference parity."
