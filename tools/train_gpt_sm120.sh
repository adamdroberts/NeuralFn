...
COMPILED_SM120_LAUNCHER_EXPLICIT="${NFN_NATIVE_SM120_CLI:-}"
USE_COMPILED_SM120_LAUNCHER="${NFN_SM120_USE_COMPILED_LAUNCHER:-1}"
AUTO_REBUILD_NATIVE="${NFN_NATIVE_GPT_AUTO_REBUILD:-${NFN_SM120_AUTO_REBUILD:-0}}"
...
ensure_default_native_trainer_current() {
  case "${AUTO_REBUILD_NATIVE,,}" in
    ...
    "0"|"false"|"no"|"off")
      ...
      ;;
    *)
      echo "Invalid NFN_NATIVE_GPT_AUTO_REBUILD/NFN_SM120_AUTO_REBUILD='${AUTO_REBUILD_NATIVE}'" >&2
      exit 2
      ;;
  esac
...
ensure_default_compiled_launcher_current() {
  ...
  if [[ -z "$COMPILED_SM120_LAUNCHER_EXPLICIT" ]]; then
    bash "$ROOT_DIR/tools/build_train_gpt_sm120_cli.sh" "$COMPILED_SM120_LAUNCHER" >/dev/null
  fi
}
...
EVAL_EVERY_STEPS="$(env_or_alias3 NFN_NATIVE_GPT_EVAL_EVERY_STEPS NFN_SM120_NATIVE_EVAL_EVERY_STEPS NFN_SM120_EVAL_EVERY_STEPS 5000)"
...
CHECKPOINT_EVERY="$(env_or_alias3 NFN_NATIVE_GPT_CHECKPOINT_EVERY NFN_SM120_NATIVE_CHECKPOINT_EVERY NFN_SM120_CHECKPOINT_EVERY 5000)"
...
LR_SCHEDULE="$(env_or_alias3 NFN_NATIVE_GPT_LR_SCHEDULE NFN_SM120_NATIVE_LR_SCHEDULE NFN_SM120_LR_SCHEDULE cosine)"
...
WARMUP_STEPS="$(env_or_alias3 NFN_NATIVE_GPT_WARMUP_STEPS NFN_SM120_NATIVE_WARMUP_STEPS NFN_SM120_WARMUP_STEPS 60)"
MAX_STEPS="$(env_or_alias3 NFN_NATIVE_GPT_MAX_STEPS NFN_SM120_NATIVE_MAX_STEPS NFN_SM120_MAX_STEPS 20000)"
TRAIN_LOSS_EVERY_STEPS="$(env_or_alias3 NFN_NATIVE_GPT_TRAIN_LOSS_EVERY_STEPS NFN_SM120_NATIVE_TRAIN_LOSS_EVERY_STEPS NFN_SM120_TRAIN_LOSS_EVERY_STEPS 250)"
SEQ_LEN_EXPLICIT=0
...
llm.kittens/train-sm120.sh, with NeuralFn validation batches on the same cadence:
  eval=5000 train_loss=250 sample=20000 generate=144 batch=64 seq=1024 tokens/step=524288
  weight_decay=0.1 beta1=0.9 beta2=0.95 adam_eps=1e-8 grad_clip_norm=1.0
  lr=0.0006 lr_schedule=cosine final_lr_fraction=0 warmup=60 checkpoint=5000
