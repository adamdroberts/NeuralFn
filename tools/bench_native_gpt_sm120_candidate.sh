...
WARMUP="$(env_or_alias5 NFN_SM120_NATIVE_WARMUP NFN_SM120_NATIVE_CANDIDATE_WARMUP NFN_SM120_CANDIDATE_WARMUP NFN_SM120_PARITY_WARMUP NFN_SM120_WARMUP 5)"
...
LONG_RUN_DEFER_PREWARM_MIN_WARMUP="$(env_or_alias NFN_SM120_NATIVE_LONG_RUN_DEFER_PREWARM_MIN_WARMUP NFN_SM120_CANDIDATE_LONG_RUN_DEFER_PREWARM_MIN_WARMUP 2)"
...
case "${CANDIDATE_PROFILE,,}" in
  ...
  "long_run_defer_prewarm"|"long-run-defer-prewarm"|"defer_prewarm_long_run"|"defer-prewarm-long-run")
    ...
    ACCEPTED_CANDIDATE_REASON="CUDA 13.3.33 dedicated RTX 5090 2026-06-29 same-script llm.kittens reference rerun keeps the long-run deferred-prewarm policy as the full-training default. It reduced setup_wall_ms to 0.673973x and startup_plus_steady_state_step_wall_ms to 0.926006x versus the old eager native route, kept steady-state CUDA-event step time inside the native gate at 1.000609x, and beat the llm.kittens reference steady-state CUDA-event step at 0.998998x. The expected deferred first-step cost remains visible at 1.095859x versus old native and 1.086035x versus llm.kittens, so the profile gates steady-state separately from first-step timing."
  ...
  "short_run_forced_prewarm"|"short-run-forced-prewarm"|"force_short_run_prewarm"|"force-short-run-prewarm"|"qkv_lm_head_forced_prewarm"|"qkv-lm-head-forced-prewarm")
    REJECTED_CANDIDATE_PROFILE="$CANDIDATE_PROFILE"
    REJECTED_CANDIDATE_REASON="CUDA 13.3.33 dedicated RTX 5090 2026-06-29 20-step, 3-sample gate forced TK QKV first-use prewarm and LM-head CUDA Graph prewarm back on for the short-run auto-fast-start path. It improved train-loop wall to 0.997818x, first-step CUDA-event time to 0.910765x, and tokens/sec to 1.002186x, but rejected default promotion because setup_wall_ms regressed to 1.504017x, startup_plus_train_loop_wall_ms to 1.002755x, steady-state CUDA-event time to 1.002916x, and candidate-over-llm.kittens train-loop wall stayed at 1.002051x with tokens/sec 0.997300x."
    CANDIDATE_NOTE="Forces the two throughput prewarms skipped by short-run auto fast-startup so first-step effects can be separated from steady-state and setup regressions."
  ...
  "tk_qkv_forward_prewarm_49152"|"tk-qkv-forward-prewarm-49152"|"qkv_forward_tk_prewarm_49152"|"qkv-forward-tk-prewarm-49152")
    REJECTED_CANDIDATE_PROFILE="$CANDIDATE_PROFILE"
    REJECTED_CANDIDATE_REASON="CUDA 13.3.33 dedicated RTX 5090 2026-06-28 3-step, 1-sample stage-timed probe limited TK QKV first-use prewarm to 49152 rows. It improved setup_wall_ms to 0.978456x and startup_plus_first_step_wall_ms to 0.998249x, but rejected default promotion because train_loop_wall_ms_per_step regressed to 1.002539x, first-step CUDA-event time to 1.004288x, stage.block_forward.attention.qkv.first_step_avg_ms to 1.063042x, and train_tokens_per_second fell to 0.997467x."
  ...
  "store_mlp_blocks9_tail"|"store-mlp-blocks9-tail"|"stored_mlp_blocks9_tail"|"stored-mlp-blocks9-tail"|"mlp_activation_blocks9_tail"|"mlp-activation-blocks9-tail")
    REJECTED_CANDIDATE_PROFILE="$CANDIDATE_PROFILE"
    REJECTED_CANDIDATE_REASON="Unpromoted stored-MLP startup-memory diagnostic: cuts stored MLP activation blocks 12->9 and places the saved blocks at the tail of the transformer. This fills the measurement gap between the rejected 9-block head profile and the rejected 6-block tail profile; keep rejected until a same-script gate proves startup improvement without train-loop or llm.kittens parity regression."
  ...
  "bf16_block_dweight_staging"|"bf16-block-dweight-staging"|"block_dweight_bf16_staging"|"block-dweight-bf16-staging")
    REJECTED_CANDIDATE_PROFILE="$CANDIDATE_PROFILE"
    REJECTED_CANDIDATE_REASON="Rejected BF16 block dWeight staging diagnostic: enables NFN_NATIVE_GPT_BF16_BLOCK_DWEIGHT_STAGING=1 so QKV and MLP FC BF16/BF16 dWeight kernels write BF16 staging buffers that feed BF16-gradient global-norm and AdamW kernels directly. A 2026-06-29 one-step dedicated RTX 5090 check proved the route by moving adamw_bf16_param_bf16_grad_descriptor_count from 0 to 24, linear_bf16_gemm_count from 605 to 797, and linear_cublaslt_bgrad_gemm_count from 384 to 192, but rejected it because train_loop_wall_ms_per_step regressed to 1.015619x and train_tokens_per_second fell to 0.984619x. Keep rejected until a same-script gate proves train-loop, stage, and llm.kittens reference parity."
