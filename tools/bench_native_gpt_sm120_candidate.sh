...
case "${CANDIDATE_PROFILE,,}" in
  ...
  "lm_head_graph_body_cublaslt_dhidden"|"lm-head-graph-body-cublaslt-dhidden"|"lm_head_cooperative_graph_body_cublaslt_dhidden"|"lm-head-cooperative-graph-body-cublaslt-dhidden")
    REJECTED_CANDIDATE_PROFILE="$CANDIDATE_PROFILE"
    REJECTED_CANDIDATE_REASON="CUDA 13.3.33 dedicated RTX 5090 2026-06-28 3-step, 1-sample stage-timed gate routed only the cooperative CUDA graph body dHidden GEMM through the existing strided cuBLASLt kernel while leaving dWeight on the default Tile launcher, but rejected it at 1.075566x train-loop wall, 1.074314x steady-state CUDA-event timing, 1.328682x LM-head backward, and 1.475505x cooperative LM-head substage time."
  ...
  "lm_head_true_fused_tile16_wmma_warp32"|"lm-head-true-fused-tile16-wmma-warp32"|"lm_head_true_fused_cooperative_tile16_wmma_warp32"|"lm-head-true-fused-cooperative-tile16-wmma-warp32")
    REJECTED_CANDIDATE_PROFILE="$CANDIDATE_PROFILE"
    REJECTED_CANDIDATE_REASON="Unpromoted strict LM-head true-fused one-warp WMMA diagnostic: builds the candidate Tile ops library with NFN_TILE_CUDA_LM_HEAD_TRUE_FUSED_MAT_TILE=16, NFN_TILE_CUDA_LM_HEAD_TRUE_FUSED_WMMA=1, and NFN_TILE_CUDA_LM_HEAD_TRUE_FUSED_THREADS=32, then forces CE threads to 32 and runs the focused LM-head preflight before any full GPT loop. Keep rejected until it beats the current CUDA Graph wrapper and llm.kittens reference gates."
