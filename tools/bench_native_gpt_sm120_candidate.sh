...
case "${CANDIDATE_PROFILE,,}" in
  ...
  "mlp_fc_dinput_before_dweight"|"mlp-fc-dinput-before-dweight")
    REJECTED_CANDIDATE_PROFILE="$CANDIDATE_PROFILE"
    REJECTED_CANDIDATE_REASON="CUDA 13.3.33 dedicated RTX 5090 2026-06-25 post-512-bias-default 3-step, 2-sample stage-timed rerun moved block_backward_mlp_fc_dinput_before_dweight_count from 0 to 288 and improved train_loop_wall_ms_per_step to 0.979771x, but rejected default promotion because steady-state CUDA-event timing regressed to 1.002092x, stage.lm_head_backward.total_ms regressed to 1.000824x, and stage.block_backward.mlp_fc.total_ms regressed to 1.040582x."
  ...
  "lm_head_graph_prewarm"|"lm-head-graph-prewarm"|"lm_head_cooperative_graph_prewarm"|"lm-head-cooperative-graph-prewarm")
    CANDIDATE_NOTE="CUDA 13.3.33 dedicated RTX 5090 2026-06-25 gated refresh promoted graph prewarm as the native default because the 3-step same-script gate eliminated runtime LM-head graph capture, improved train_loop_wall_ms_per_step to 0.976542x, train_tokens_per_second to 1.024025x, stage.lm_head_backward.total_ms to 0.967861x, stage.block_backward.total_ms to 0.974829x, and stage.block_backward.mlp_proj.total_ms to 0.845548x versus the lazy-capture route while passing the 1.002 steady-state CUDA-event tolerance. The llm.kittens parity gate still misses strict steady-state CUDA-event timing, so the remaining implementation target is a true fused LM-head classifier-backward Tile kernel."
    BASELINE_ENV_RAW="${BASELINE_ENV_RAW:+$BASELINE_ENV_RAW }NFN_NATIVE_GPT_LM_HEAD_COOPERATIVE_GRAPH_PREWARM=0 NFN_NATIVE_GPT_PREWARM_CUBLAS_HANDLE=0 NFN_NATIVE_GPT_PREWARM_BF16_WORKSPACE=0"
    CANDIDATE_ENV_RAW="${CANDIDATE_ENV_RAW:+$CANDIDATE_ENV_RAW }NFN_NATIVE_GPT_LM_HEAD_COOPERATIVE_GRAPH_PREWARM=1 NFN_NATIVE_GPT_PREWARM_CUBLAS_HANDLE=1 NFN_NATIVE_GPT_PREWARM_BF16_WORKSPACE=1"
    AUTO_DISABLE_METRIC_RATIO_GATES=1
