...
case "${CANDIDATE_PROFILE,,}" in
  ...
  "lm_head_true_fused_cooperative"|"lm-head-true-fused-cooperative"|"lm_head_true_fused"|"lm-head-true-fused")
    REJECTED_CANDIDATE_PROFILE="$CANDIDATE_PROFILE"
    REJECTED_CANDIDATE_REASON="CUDA 13.3.33 dedicated RTX 5090 2026-06-28 focused trainer-chunk preflight proved the default 32x32 strict body with candidate_true_fused_capability=true and path class strict-true-fused-tile-kernel, but rejected it at 6.708146x candidate/current-wrapper and 22.033921x candidate/reference-summed time. The strict body remained 753.913597 ms slower than the reference CE+dHidden+dWeight components. The 2026-06-27 full-loop gate also regressed train_loop_wall_ms_per_step to 5.991992x, train_tokens_per_second to 0.166890x, stage.lm_head_backward.total_ms to 22.660619x, and stage.lm_head_backward.cooperative.total_ms to 32.243288x versus the default CUDA Graph wrapper. Keep this profile rejected until the strict body beats focused and full-loop/reference gates."
