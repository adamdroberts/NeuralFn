...
WARMUP="$(env_or_alias5 NFN_SM120_NATIVE_WARMUP NFN_SM120_NATIVE_CANDIDATE_WARMUP NFN_SM120_CANDIDATE_WARMUP NFN_SM120_PARITY_WARMUP NFN_SM120_WARMUP 7)"
...
case "${CANDIDATE_PROFILE,,}" in
  ...
  "tk_qkv_forward_prewarm_1row"|"tk-qkv-forward-prewarm-1row"|"qkv_forward_tk_prewarm_1row"|"qkv-forward-tk-prewarm-1row")
    REJECTED_CANDIDATE_PROFILE="$CANDIDATE_PROFILE"
    REJECTED_CANDIDATE_REASON="CUDA 13.3.33 dedicated RTX 5090 2026-06-29 7-warmup, 3-step, 2-sample long-run deferred-prewarm rerun kept the one-row TK QKV first-use prewarm rejected. Against the current long-run no-QKV-prewarm baseline it improved train_loop_wall_ms_per_step to 0.978921x, first-step CUDA-event time to 0.941562x, steady-state CUDA-event time to 0.999721x, and train tokens/sec to 1.021530x, but failed the startup contract because setup_wall_ms regressed to 1.380026x, startup_plus_first_step_wall_ms to 1.008787x, and startup_plus_steady_state_step_wall_ms to 1.063530x. Candidate-over-llm.kittens steady-state timing passed at 0.997438x, but train tokens/sec trailed the reference at 0.990832x. Keep it as a diagnostic route until it beats startup-plus-first-step and reference throughput gates."
