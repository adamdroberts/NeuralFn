...
case "${CANDIDATE_PROFILE,,}" in
  ...
  "token_weight_padded_bf16_pattern"|"token-weight-padded-bf16-pattern"|"token_weight_padded_pattern"|"token-weight-padded-pattern")
    REJECTED_CANDIDATE_PROFILE="$CANDIDATE_PROFILE"
    REJECTED_CANDIDATE_REASON="CUDA 13.3.33 dedicated RTX 5090 2026-06-28 3-step, 2-sample same-script recheck changed token_weight_init_strategy to device-vector4-strided-power2-deterministic-fused-bf16-pattern-shadow-padded-zero and improved setup_wall_ms to 0.971027x, startup_plus_train_loop_wall_ms to 0.997089x, train_loop_wall_ms_per_step to 0.999768x, and candidate-over-llm.kittens train_loop_wall_ms_per_step to 0.999856x, but kept default promotion rejected because the target setup.token_weight_init.total_ms gate failed at 1.001098x and candidate-over-reference first-step CUDA-event timing regressed to 1.000752x. Treat the setup win as allocation noise until the token-init and first-step gates both pass."
