...
case "${CANDIDATE_PROFILE,,}" in
  ...
  "lm_head_graph_prewarm"|"lm-head-graph-prewarm"|"lm_head_cooperative_graph_prewarm"|"lm-head-cooperative-graph-prewarm")
    REJECTED_CANDIDATE_PROFILE="$CANDIDATE_PROFILE"
    REJECTED_CANDIDATE_REASON="CUDA 13.3.33 dedicated RTX 5090 2026-06-25 post-reinstall rerun proved setup prewarm still eliminates runtime LM-head CUDA Graph captures and preserves LM-head shape telemetry. The 3-step, 2-sample reference-including gate improved native-vs-native train_loop_wall_ms_per_step to 0.985820x and train_tokens_per_second to 1.014494x, but rejected default promotion because candidate-over-llm.kittens regressed train_loop_wall_ms_per_step to 1.004960x, steady-state CUDA-event timing to 1.001805x, and train_tokens_per_second to 0.993865x. Keep graph prewarm diagnostic-only until the reference steady-state and throughput gates pass."
