...
ALLOW_LOW_LONG_RUN_DEFER_PREWARM_DIAGNOSTIC="$(env_or_alias5 NFN_SM120_NATIVE_ALLOW_LOW_LONG_RUN_DEFER_PREWARM_DIAGNOSTIC NFN_SM120_NATIVE_CANDIDATE_ALLOW_LOW_LONG_RUN_DEFER_PREWARM_DIAGNOSTIC NFN_SM120_CANDIDATE_ALLOW_LOW_LONG_RUN_DEFER_PREWARM_DIAGNOSTIC NFN_SM120_PARITY_ALLOW_LOW_LONG_RUN_DEFER_PREWARM_DIAGNOSTIC NFN_SM120_ALLOW_LOW_LONG_RUN_DEFER_PREWARM_DIAGNOSTIC 0)"
...
case "${CANDIDATE_PROFILE,,}" in
  ...
  "long_run_forced_prewarm"|"long-run-forced-prewarm"|"long_run_qkv_lm_head_forced_prewarm"|"long-run-qkv-lm-head-forced-prewarm")
    ...
    CANDIDATE_NOTE="Compares the explicit long-run deferred benchmark route against forcing both known first-use prewarms back on. This profile should stay rejected until the setup cost is reduced or overlapped enough for startup-plus gates to pass while retaining the reference wins."
  ...
  "long_run_qkv_forward_prewarm"|"long-run-qkv-forward-prewarm"|"long_run_qkv_forward_full_prewarm"|"long-run-qkv-forward-full-prewarm")
    ...
    CANDIDATE_NOTE="Compares the explicit long-run deferred benchmark route against forcing TK QKV first-use prewarm back on. This profile keeps long-run warmup and step floors unless the caller explicitly overrides them for a bounded reproduction."
  ...
  "long_run_qkv_forward_async_prewarm"|"long-run-qkv-forward-async-prewarm"|"long_run_qkv_async_prewarm"|"long-run-qkv-async-prewarm")
    ...
    CANDIDATE_NOTE="Compares the benchmark long-run deferred route with TK QKV prewarm disabled against the opt-in async QKV prewarm route. The candidate threshold is one step so short benchmark runs exercise the same explicit policy branch, and the profile gates steady-state/reference metrics separately from first-step timing."
  ...
  "embedding_bf16_shadow"|"embedding-bf16-shadow"|"token_embedding_bf16_shadow"|"token-embedding-bf16-shadow"|"bf16_embedding_shadow"|"bf16-embedding-shadow")
    REJECTED_CANDIDATE_PROFILE="$CANDIDATE_PROFILE"
    REJECTED_CANDIDATE_REASON="Dedicated RTX 5090 2026-06-30 3-step, 2-sample same-script rerun with 5 warmup pairs changed embedding_bf16_shadow_enabled false->true and embedding_residual_strategy to fused-token-u16-bf16-shadow-position-residual. It improved setup.token_weight_init.total_ms to 0.993064x and setup.parameter_initialization.total_ms to 0.993872x, but rejected default promotion because train_loop_wall_ms_per_step regressed to 1.002959x, steady-state CUDA-event timing to 1.003632x, setup_wall_ms regressed to 1.039402x, and train_tokens_per_second fell to 0.997053x."
  ...
  "lm_head_cooperative_sequence_wrapper"|"lm-head-cooperative-sequence-wrapper"|"lm_head_cooperative_cuda_graph_off"|"lm-head-cooperative-cuda-graph-off")
    REJECTED_CANDIDATE_PROFILE="$CANDIDATE_PROFILE"
    REJECTED_CANDIDATE_REASON="Dedicated RTX 5090 2026-06-30 20-warmup, 10-step, 2-sample native-only same-script rerun rejected the cooperative LM-head sequence wrapper as the default over cached CUDA Graph replay. The route changed as intended by dropping graph replay counters from 240 to 0 and increasing sequence launches from 0 to 240. It improved setup_wall_ms to 0.957808x and startup_plus_first_step_wall_ms to 0.991651x, but failed the throughput gates: train_loop_wall_ms_per_step regressed to 1.000449x and train_tokens_per_second fell to 0.999552x. Keep cached CUDA Graph replay as the native default; use this profile only as an explicit sequence-wrapper bisection."
...
case "${ALLOW_LOW_LONG_RUN_DEFER_PREWARM_DIAGNOSTIC,,}" in
  ...
  "0"|"false"|"no"|"off"|"")
    ...
    ;;
  *)
    echo "Unsupported NFN_SM120_NATIVE_ALLOW_LOW_LONG_RUN_DEFER_PREWARM_DIAGNOSTIC value: $ALLOW_LOW_LONG_RUN_DEFER_PREWARM_DIAGNOSTIC" >&2
    exit 2
    ;;
esac
