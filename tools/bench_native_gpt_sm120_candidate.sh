...
case "${CANDIDATE_PROFILE,,}" in
  ...
  "lm_head_true_fused_cooperative"|"lm-head-true-fused-cooperative"|"lm_head_true_fused"|"lm-head-true-fused")
    REJECTED_CANDIDATE_PROFILE="$CANDIDATE_PROFILE"
    REJECTED_CANDIDATE_REASON="CUDA 13.3.33 dedicated RTX 5090 2026-06-26 1-step stage-timed probe proved the route by moving lm_head_classifier_true_fused_launch_count from 0 to 16 and the path class to strict-true-fused-tile-kernel, but rejected the original scalar diagnostic body because train_loop_wall_ms_per_step regressed to 7.217785x, stage.lm_head_backward.total_ms to 30.473055x, and stage.lm_head_backward.cooperative.total_ms to 43.503750x versus the default CUDA Graph wrapper. A later 32x32 tiled diagnostic body still failed the focused trainer-chunk gate at 7.046530x versus the default wrapper, so keep this profile rejected until the strict body beats full-loop/reference gates."
