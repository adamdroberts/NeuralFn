...
LONG_RUN_DEFER_PREWARM_MIN_WARMUP="$(env_or_alias NFN_SM120_NATIVE_LONG_RUN_DEFER_PREWARM_MIN_WARMUP NFN_SM120_CANDIDATE_LONG_RUN_DEFER_PREWARM_MIN_WARMUP 7)"
...
case "${CANDIDATE_PROFILE,,}" in
  ...
  "tk_qkv_forward_prewarm_49152"|"tk-qkv-forward-prewarm-49152"|"qkv_forward_tk_prewarm_49152"|"qkv-forward-tk-prewarm-49152")
    REJECTED_CANDIDATE_PROFILE="$CANDIDATE_PROFILE"
    REJECTED_CANDIDATE_REASON="CUDA 13.3.33 dedicated RTX 5090 2026-06-29 post-LM-head-sequence-default 7-warmup, 3-step, 2-sample rerun limited TK QKV first-use prewarm to 49152 rows. With LM-head CUDA Graph replay disabled on both sides, it improved startup_plus_first_step_wall_ms to 0.995515x, startup_plus_train_loop_wall_ms to 0.998019x, train_loop_wall_ms_per_step to 0.977976x, first-step CUDA-event time to 0.938976x, forward-QKV first-step timing to 0.406568x, and train tokens/sec to 1.022522x, but rejected default promotion because setup_wall_ms regressed to 1.307975x. Candidate-over-llm.kittens steady-state timing passed at 0.997260x, but overall train tokens/sec still trailed the reference at 0.991683x. Keep it diagnostic until setup wall no longer regresses."
