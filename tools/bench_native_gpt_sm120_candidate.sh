...
case "${CANDIDATE_PROFILE,,}" in
  ...
  "concurrent_parameter_init"|"concurrent-parameter-init"|"parallel_parameter_init"|"parallel-parameter-init"|"concurrent_token_parameter_init"|"concurrent-token-parameter-init")
    REJECTED_CANDIDATE_PROFILE="$CANDIDATE_PROFILE"
    REJECTED_CANDIDATE_REASON="CUDA 13.3.33 dedicated RTX 5090 2026-06-29 3-sample, 10-step native-only paired gate compared concurrent token-weight initialization plus non-token parameter fill against the current serial setup path after moving the initial BF16 token refresh behind the concurrent init gate. It rejected default promotion because train_loop_wall_ms_per_step regressed to 1.001353x mean, train_loop_cuda_event_steady_state_wall_ms_per_step regressed to 1.001590x mean, and train_tokens_per_second fell to 0.998651x, despite setup_wall_ms improving to 0.983885x and startup_plus_first_step_wall_ms improving to 0.995546x."
