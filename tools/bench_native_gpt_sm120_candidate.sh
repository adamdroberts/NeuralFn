...
case "${CANDIDATE_PROFILE,,}" in
  ...
  "lm_head_graph_prewarm"|"lm-head-graph-prewarm"|"lm_head_cooperative_graph_prewarm"|"lm-head-cooperative-graph-prewarm")
    REJECTED_CANDIDATE_PROFILE="$CANDIDATE_PROFILE"
    REJECTED_CANDIDATE_REASON="CUDA 13.3.33 dedicated RTX 5090 2026-06-25 3-step, 2-sample same-script refresh eliminated runtime LM-head graph capture and improved train_loop_wall_ms_per_step to 0.995921x, train_tokens_per_second to 1.004099x, and stage.lm_head_backward.total_ms to 0.966626x versus lazy capture, but rejected promotion because steady-state CUDA-event timing regressed to 1.002619x, stage.block_backward.total_ms regressed to 1.009095x, and MLP FC backward regressed. Keep graph prewarm opt-in diagnostic plumbing until a real fused LM-head classifier-backward Tile kernel replaces the CUDA Graph wrapper."
