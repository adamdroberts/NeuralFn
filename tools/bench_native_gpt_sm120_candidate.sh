...
case "${CANDIDATE_PROFILE,,}" in
  ...
  "tk_qkv_forward_prewarm_1row"|"tk-qkv-forward-prewarm-1row"|"qkv_forward_tk_prewarm_1row"|"qkv-forward-tk-prewarm-1row")
    REJECTED_CANDIDATE_PROFILE="$CANDIDATE_PROFILE"
    REJECTED_CANDIDATE_REASON="CUDA 13.3.33 dedicated RTX 5090 2026-06-26 3-step, 1-sample stage-timed gate proved the one-row route by moving linear_tk_qkv_first_use_prewarm_success_count from 0 to 1 and setting effective_rows to 1. It improved NeuralFn train-loop wall to 0.986095x and forward-QKV first-step avg to 0.477316x, but setup regressed to 1.415877x, total wall to 1.018835x, and strict llm.kittens reference gates still failed at 1.010943x train-loop wall, 1.015312x first-step CUDA-event timing, 1.008765x steady-state CUDA-event timing, and 0.988815x tokens/sec."
    BASELINE_ENV_RAW="${BASELINE_ENV_RAW:+$BASELINE_ENV_RAW }NFN_NATIVE_GPT_PREWARM_TK_QKV_FORWARD=0"
