...
case "${CANDIDATE_PROFILE,,}" in
  ...
  "lm_head_graph_prewarm"|"lm-head-graph-prewarm"|"lm_head_cooperative_graph_prewarm"|"lm-head-cooperative-graph-prewarm")
    REJECTED_CANDIDATE_PROFILE="$CANDIDATE_PROFILE"
    REJECTED_CANDIDATE_REASON="CUDA 13.3.33 dedicated RTX 5090 2026-06-25 reruns proved setup prewarm eliminates runtime LM-head CUDA Graph captures and preserves LM-head shape telemetry. The 10-step, 2-sample native-vs-native gate improved train_loop_wall_ms_per_step to 0.990304x, first-step CUDA-event time to 0.913676x, and train_tokens_per_second to 1.009811x, but rejected default promotion because steady-state CUDA-event timing was still 1.000369x. The reference-including 3-step run beat llm.kittens total wall at 0.991761x but also missed the strict reference steady-state gate at 1.000990x. Keep graph prewarm diagnostic-only until the long-loop steady gate passes."
