...
case "${CANDIDATE_PROFILE,,}" in
  ...
  "lm_head_true_fused_cooperative"|"lm-head-true-fused-cooperative"|"lm_head_true_fused"|"lm-head-true-fused")
    REJECTED_CANDIDATE_PROFILE="$CANDIDATE_PROFILE"
    REJECTED_CANDIDATE_REASON="CUDA 13.3.33 dedicated RTX 5090 2026-06-26 1-step stage-timed probe proved the route by moving lm_head_classifier_true_fused_launch_count from 0 to 16 and the path class to strict-true-fused-tile-kernel, but rejected the current scalar diagnostic body because train_loop_wall_ms_per_step regressed to 7.217785x, stage.lm_head_backward.total_ms to 30.473055x, and stage.lm_head_backward.cooperative.total_ms to 43.503750x versus the default CUDA Graph wrapper. Keep rejected until a tiled production body replaces the scalar CE+dHidden+dWeight loops and passes full-loop/reference gates."
    CANDIDATE_NOTE="Runs the trainer-sized full GPT loop with the opt-in strict true-fused cooperative LM-head body instead of the diagnostic CUDA Graph wrapper. The current body is a scalar diagnostic, not an optimized production Tile kernel; this profile must remain rejected until a tiled replacement passes the promotion gate."
