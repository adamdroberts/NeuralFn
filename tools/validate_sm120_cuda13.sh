...
if [[ -n "${NFN_SM120_CUDA13_PARITY_MAX_CANDIDATE_RATIO:-}" ]]; then
  PARITY_MAX_CANDIDATE_RATIO="${NFN_SM120_CUDA13_PARITY_MAX_CANDIDATE_RATIO}"
elif [[ "${PARITY_STEPS}" =~ ^[0-9]+$ && "${PARITY_STEPS}" -lt 2 ]]; then
  PARITY_MAX_CANDIDATE_RATIO=""
else
  PARITY_MAX_CANDIDATE_RATIO="median:train_loop_cuda_event_steady_state_wall_ms_per_step=1.003"
fi
...
checks = [
    ...
    (
        ...
        "runtime contract must keep auto fast-startup smokes on the skip-throughput-prewarms policy",
    ),
    (
        ...
        "runtime contract must keep TK QKV first-use prewarm skipped on auto fast-startup smokes",
    ),
    (
...
case "${NFN_SM120_CUDA13_RUN_LM_HEAD_BENCH:-1}" in
  1|true|TRUE|yes|YES|on|ON)
    run_step env \
      ...
      NFN_LM_HEAD_BACKWARD_WARMUP="${NFN_SM120_CUDA13_LM_HEAD_WARMUP:-2}" \
...
case "${NFN_SM120_CUDA13_RUN_PARITY:-1}" in
  1|true|TRUE|yes|YES|on|ON)
    run_step env \
      ...
      NFN_SM120_PARITY_MAX_CANDIDATE_RATIO="${PARITY_MAX_CANDIDATE_RATIO}" \
