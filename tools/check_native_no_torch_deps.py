...
NATIVE_BINDING_SOURCE_DEPENDENCIES = {
    ...
    "_native_inference": (
        ...
        Path("neuralfn/csrc/native_gpt2/resident_glimmer.cpp"),
        Path("neuralfn/csrc/native_gpt2/resident_glimmer.h"),
        Path("neuralfn/csrc/native_gpt2/resident_glimmer_vision.cpp"),
        Path("neuralfn/csrc/native_gpt2/resident_glimmer_vision.h"),
        Path("neuralfn/csrc/native_gpt2/resident_glimmer_assistant.cpp"),
        Path("neuralfn/csrc/native_gpt2/resident_glimmer_assistant.h"),
        Path("neuralfn/csrc/native_gpt2/resident_glimmer_cuda.cpp"),
        Path("neuralfn/csrc/native_gpt2/resident_glimmer_cuda.h"),
...
def _validate_native_template_catalog(
    *,
    ...
    report: dict[str, object] = {
        ...
        "all_template_runnable": False,
        "non_runnable_templates": [],
        "errors": [],
    }
    ...
    non_runnable_templates = [
        ...
        for template in templates
        if isinstance(template, dict) and template.get("selected_graph_native_runnable") is not True
    ]
    ...
    dataset_loop_reports: dict[str, object] = {}
    for template_name in REQUIRED_NATIVE_DATASET_LOOP_TEMPLATE_SENTINELS:
        template = by_name.get(template_name)
        template_report = {
            "passed": False,
            "status": None,
        ...
        template_errors = template_report["errors"]
        if not isinstance(template_errors, list):
            raise AssertionError("internal dataset-loop sentinel error storage mismatch")
        if template is None:
            template_errors.append("template missing from catalog")
        else:
            template_report["status"] = template.get("selected_graph_support_status")
            template_report["native_runnable"] = template.get("selected_graph_native_runnable")
            template_report["missing_requirements"] = template.get("native_training_missing_requirements")
            if template.get("selected_graph_support_status") != "native-family-dataset-loop":
                template_errors.append(
                    "status="
                    f"{template.get('selected_graph_support_status')!r}, "
                    "expected 'native-family-dataset-loop'"
                )
            if template.get("selected_graph_native_runnable") is not True:
                template_errors.append("selected_graph_native_runnable was not true")
        ...
        if template_errors:
            errors.append(f"{template_name}: {'; '.join(template_errors)}")
        dataset_loop_reports[template_name] = template_report
    ...
    report["dataset_loop_sentinels"] = dataset_loop_reports
