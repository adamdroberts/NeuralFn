...
- The trainer-facing linear backend default changed from BF16 `cublasGemmEx` to TF32 tensor-op `cublasSgemm`. Callers that require the old BF16 bridge must set `NFN_TILE_CUDA_LINEAR_BF16=1` or `NFN_NATIVE_LINEAR_BF16=1`; callers parsing JSON should update default checks from `linear_backend_strategy: "bf16-gemmex-float32-output"` or `"tf32-sgemm-fallback"` to `"tf32-sgemm-optimized"`.
