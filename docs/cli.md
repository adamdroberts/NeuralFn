...
For the composed semantic dense JEPA native train-step slice, use `nfn_semantic_dense_jepa_native_train --train-semantic-dense-jepa-loop-step --tile-ops-lib PATH` or `nfn-native-train --base-model semantic-dense-jepa-evo --native-cuda-train-semantic-dense-jepa-loop-step`; the bare semantic-dense-JEPA family command also defaults to this slice. It wraps the semantic planner/alignment AdamW train-step through compiled native C++ and returns one JSON payload with substep JSON, `torch_required: false`, `graph_editor_tensor_flow: false`, and `production_training_loop: false` until the full multi-step production trainer lands.
