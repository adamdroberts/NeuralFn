...
For the composed universal-transformer native train-step slice, use `nfn_universal_llama_native_train --train-universal-loop-step --tile-ops-lib PATH` or `nfn-native-train --base-model universal-llama --native-cuda-train-universal-loop-step`; the bare universal-transformer family command also enters this slice by default. It wraps the recurrent/ACT loop smoke through compiled native C++ and reports substep JSON without Torch or graph-editor tensor flow.
