...
completed while byte-token shard resolution, byte-patch backward, checkpointing,
and inference remain missing.
...
HNet entries now list the completed byte patch embed/merge plus
head-loss/backward/AdamW smoke while still reporting byte-token shard
resolution, byte-patch backward, checkpointing, and inference wiring as
missing.
