...
For the composed MoE-JEPA native train-step slice, use `nfn_moe_jepa_evo_native_train --train-moe-jepa-loop-step --tile-ops-lib PATH` or `nfn-native-train --base-model moe-jepa-evo --native-cuda-train-moe-jepa-loop-step`; the bare `moe-jepa-evo` family command also defaults to this slice. It runs the standard MoE full forward/backward-loop train-step and the AR+JEPA+router objective substep through compiled native C++ and returns one JSON payload with substep JSON, `torch_required: false`, `graph_editor_tensor_flow: false`, and `production_training_loop: false` until the full multi-step production trainer lands.
