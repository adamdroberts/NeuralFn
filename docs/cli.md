...
Use these fields to audit complete GPT-template native coverage. For example,
`--template-name moe_jepa_evo --print-plan` currently reports
`missing-moe-jepa-objective` and lists the standard MoE loop, JEPA
target/projector/predictor stages, latent MSE reduction, and composite
AR+JEPA+router loss work that must be implemented before that template can run
real native training.
