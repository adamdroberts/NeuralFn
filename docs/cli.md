...
For the composed Jamba native train-step slice, use `nfn_jamba_native_train --train-jamba-loop-step --tile-ops-lib PATH` or `nfn-native-train --base-model jamba --native-cuda-train-jamba-loop-step`; the bare `nfn_jamba_native_train ...` and `nfn-native-train --base-model jamba ...` paths also enter this slice by default. It wraps the Jamba layer-schedule loop smoke through compiled native C++ and reports substep JSON without Torch or graph-editor tensor flow.
