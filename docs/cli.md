...
defaulting to ordinal `0` so normal training startup does not spawn
`nvidia-smi`. Set one of those variables to `dedicated`, `auto`, or
`dedicated-auto` when you intentionally want the display-disabled GPU probe.
