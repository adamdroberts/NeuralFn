...
For the composed diffusion native train-step slice, use `nfn_diffusion_native_train --train-diffusion-loop-step --tile-ops-lib PATH` or `nfn-native-train --base-model diffusion --native-cuda-train-diffusion-loop-step`; the bare diffusion family command also enters this slice by default. It wraps the timestep/mask/objective loop smoke through compiled native C++ and reports substep JSON without Torch or graph-editor tensor flow.
...
seq2seq, Jamba, diffusion, TTT, HNet byte-LM, and universal transformer
