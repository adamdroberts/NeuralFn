...
For the composed dense JEPA native train-step slice, use `nfn_jepa_native_train --train-dense-jepa-loop-step --tile-ops-lib PATH` or `nfn-native-train --base-model dense-jepa-evo --native-cuda-train-dense-jepa-loop-step`; the bare dense-JEPA family command also defaults to this slice. It wraps the dense JEPA full forward/backward-loop train-step through compiled native C++ and returns one JSON payload with substep JSON, `torch_required: false`, `graph_editor_tensor_flow: false`, and `production_training_loop: false` until the full multi-step production trainer lands.
