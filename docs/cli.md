...
For the composed TTT native train-step slice, use `nfn_ttt_llama_native_train --train-ttt-loop-step --tile-ops-lib PATH` or `nfn-native-train --base-model ttt-llama --native-cuda-train-ttt-loop-step`; the bare TTT family command also enters this slice by default. It wraps the full transformer loop smoke through compiled native C++ and reports substep JSON without Torch or graph-editor tensor flow.
...
seq2seq, diffusion, Jamba, TTT, HNet byte-LM, and universal transformer
