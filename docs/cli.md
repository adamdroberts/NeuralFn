...
`--kernel-backend`). `tile-cuda` is the default NeuralFn-owned compiled
trainer for dense GPT. `llm-kittens` is an explicit external bridge for parity
benchmarks and migration checks.
