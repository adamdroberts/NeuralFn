...
Structurally different shipped GPT template names are selected and reported in
JSON, but return `template-geometry-native-trainer-missing` or
...
and can run the fixed dense GPT native trainer; arbitrary or incompatible custom
...
Plan and runtime JSON also include `native_geometry_contract`. The current compiled dense GPT loop reports `name: "gpt2-compatible-fixed-dense-transformer"` and `shape_source: "compiled_dense_gpt_defaults"` for preset selection or `"custom_graph_template_spec"` for compatible custom graph metadata, with model dim 768, 12 heads, head dim 64, GELU 4x MLP, absolute positions, LayerNorm, dropout 0, public vocab 50,257, padded vocab 50,304, the selected sequence length, and the selected layer count. `template_geometry_dynamic` remains `false` until the native loop is generalized to load dimensions/dropout from every preset, while `custom_graph_geometry_dynamic` is `true` only when an existing graph file exposes compatible GPT `template_spec` metadata. The same object includes `selected_template_geometry` and `geometry_matches_compiled_loop`, so selecting `nanogpt` records its requested 320-wide/5-layer/dropout-0.1 shape while still making the current fixed compiled geometry explicit.
