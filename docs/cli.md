...
forward/backward, and load-balance/AdamW smokes while still reporting the full
family block loop, JEPA or semantic objectives, checkpointing, and inference as
missing. Dense JEPA, semantic dense JEPA, and MoE+JEPA entries now list the
completed projector/predictor/latent-loss smoke while still reporting target
encoder, full objective composition, family checkpointing, and inference wiring
as missing. Semantic dense JEPA and semantic router/MoE entries now list the
