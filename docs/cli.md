...
missing-requirements list because they are runnable. Non-dense entries keep
`production-family-forward-backward-optimizer-loop` in their missing list until
the full production trainer loop is implemented, even when their smoke-backed
native coverage gates are complete. Covered families have compiled no-Torch native targets for
LLaMA/RoPE/SwiGLU, standard MoE, dense JEPA, MoE+JEPA, semantic MoE/JEPA,
Jamba, seq2seq, diffusion, TTT, HNet byte-LM, and universal transformer
