...
per-expert weights, and output gradients. Optimizer-loop, checkpoint, and
inference wiring remain part of `family-native-loop-missing`.
...
For the composed HNet native train-step slice, use `nfn_hnet_lm_native_train --train-hnet-loop-step --tile-ops-lib PATH` or `nfn-native-train --base-model hnet-lm --native-cuda-train-hnet-loop-step`; the bare HNet family command also enters this slice by default. It wraps the byte-LM loop smoke through compiled native C++ and reports substep JSON without Torch or graph-editor tensor flow.
