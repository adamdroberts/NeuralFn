...
defaulting to `dedicated`, which selects the lowest-utilization
display-disabled NVIDIA GPU through `nvidia-smi` and falls back to ordinal `0`
if that probe is unavailable. Set one of those variables to an explicit ordinal
when a run should skip the GPU selector.
