...
always `false`, but the preflight now reports the available device-side evo
mutation/selection/adoption ABI while distinguishing dense GPT-2-compatible
evo work (`native-gpt2-evo-trainer-missing`) from other shipped templates,
custom graph files, and typoed template names without importing the graph-backed
runtime.
