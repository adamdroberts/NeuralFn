...
paired native benchmarks. The 64-row profile changed the route and improved
train-loop wall to `0.998045x`, but it remains rejected because it regressed
`stage.block_backward.mlp_proj.total_ms` to `1.004276x` and
`stage.lm_head_backward.total_ms` to `1.000446x`. JSON reports
