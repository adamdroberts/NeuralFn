...
## TurboQuant packed-attention feature ABI

The trainer-facing sidecar has an additive, versioned CUDA attention ABI for
the resident CPU-v1 TurboQuant record format. It leaves the base Tile ABI at v1
and exports `nfn_native_tile_turboquant_attention_abi_version`,
`nfn_native_tile_turboquant_attention_forward_v1`, plus launch counter/reset
probes. The size-prefixed descriptor supplies device pointers for packed
historical K/V, exact current K/V, queries, shared float64 rotation/QJL/codebook
tables, and explicit batch/layer/head/record-stride geometry.

The CUDA kernel reads unaligned float32 record headers and mixed 4/3-bit MSE or
3/2-bit-plus-sign QJL payloads in place. It supports MHA/GQA, adds the exact
current row to the same deterministic online softmax, and chunks long sequences
without materializing a dequantized cache matrix. V1 accepts total contexts
through 16,384 rows.

Run the fail-honest agreement gate with a visible CUDA device:
...
This rebuilds and executes both the default fast-math/TK sidecar and the strict
sidecar. The RTX 5090 kernel gate passes 19 tests across both profiles, MHA/GQA,
dimensions 8/64/128, current-only attention, 1023/1024/1025 and 4K/16K
boundaries, deterministic replay/counters, invalid descriptors, and portable/
native-CPU agreement.

Reviewed dense-v5 Native Execution artifacts now advertise a separate
`turboquant_tile_attention` feature gate for compatible 16K/even-head geometry,
and the resident engine consumes this ABI only after an explicit `tile-cuda`
request. Build the resident extension without linking CUDA, and build a fresh
strict sidecar:
...
`build/libnfn_native_train_tile_ops_strict.so`. The resident bridge loads CUDA
and this sidecar dynamically, uploads shared tables once per model/profile,
owns packed device cache/stream/scratch per session, and uploads each newly
encoded row. CPU remains the default and an explicit Tile request has no CPU
fallback. Run the direct binding and public SDK lifecycle gate with:
...
The RTX 5090 resident gate passes both profiles and requires positive GPU
launch/upload/H2D/D2H telemetry with zero CPU compressed-attention calls.
For transfer-inclusive timing, cache/VRAM, NLL/perplexity, and greedy agreement,
run `tools/bench_native_resident_turboquant.py` with an explicit artifact,
fresh resident binding, strict sidecar, CUDA runtime, and matching token corpus;
see [Resident Native Inference](native-inference.md#resident-turboquant-benchmark).
The harness isolates timing from quality/VRAM and always emits
`speedup_claimed: false`. Its 2026-08-08 RTX 5090 synthetic 1K/4K/16K
calibration proved the measurement path and exact Tile/CPU-profile agreement,
but the tiny launch-dominated fixture made Tile slower than CPU and cannot
support a product speed or quality-neutrality claim. A representative trained
artifact/corpus run remains required.

The trainer-facing rotary embedding symbol accepts total element count as its
shape argument, then adapts that count to the shared batch-based CUDA Tile
launcher. This is different from the Python extension helper, which passes
`batch` directly from `[B,H,S,D]` tensors. Compact native tensors such as
DeepSeek-V3 MLA split RoPE query/key buffers rely on the element-count ABI and
the kernel's paired half-dimension bounds checks.
...
The trainer-facing ABI also exposes
`nfn_native_tile_extract_diagonal_float32(matrix, diagonal, dim, stream)`.
Native family trainers use it to reduce affine RMSNorm weight-gradient
matrices on device before persistent optimizer accumulation, avoiding a
device-to-host copy in the production full-geometry path; Seq2seq and HNet
runtime JSON report this as
`specialty_hnet_seq2seq_norm_device_gradient_accumulation: true`. Seq2seq
also stages mixed encoder/cross-QKV slices into stacked device gradients before
optimizer accumulation and reports
`specialty_seq2seq_stacked_qkv_device_gradient_accumulation: true`.
Direct specialty gradient paths skip the former post-backward full-device
synchronization before handing device gradients to the persistent optimizer and
report `specialty_direct_gradient_sync_elision: true`; full-step capture still
reports the remaining host-side launch orchestration and reporting readback
blockers separately.
Semantic route-evo production refreshes apply the adopted router candidate
through `nfn_native_tile_evo_adopt_candidate_float32` into the live
`router.weight` device buffer and report
`full_family_semantic_route_evo_device_adoption: true`.
Semantic JEPA target-topic route distillation uses
`nfn_native_tile_semantic_target_topic_distillation_backward_float32` to add
reduced target-topic router-logit gradients and loss before semantic
hash/table backward, and reports
`full_family_semantic_target_topic_route_distillation_device_backward: true`.
Fresh semantic-family parameter stores initialize `semantic_hash.proj.weight`
with the NumPy-compatible `RandomState(42).randn(tables, planes, dims)` sequence
used by `SemanticChunkHasherStage` and report
`full_family_semantic_hash_projection_seeded_exact: true`; checkpoint resume
preserves the stored projection.
Semantic-router MoE free-expert routing uses
`nfn_native_tile_semantic_free_expert_projection_float32` and
`nfn_native_tile_semantic_free_expert_projection_backward_float32` to project the
native semantic vector into free-expert route logits and backpropagate selected
route gradients into `semantic_router.free_head.weight`; the
`nfn_native_tile_semantic_router_bias_add_float32` and
`nfn_native_tile_semantic_router_bias_backward_float32` ABI adds/reduces
`semantic_router.shared_logits` and `semantic_router.free_head.bias`. The
runtime reports
`full_family_semantic_free_expert_projection_device_forward_backward: true`.
The free-expert Tile ABI now separates semantic route vocabulary width from
semantic vector width, allowing canonical per-term routes to score the appended
signature coordinate while hash routing continues to use the seeded 86-d topic
projection. The native trainer reports
`full_family_semantic_chunk_projector_signature_coordinate_device_forward_backward: true`
when `semantic_projector.sig_head.weight` participates in forward/backward.
Always-on shared expert routing uses the semantic router bias ABI for the
template's scalar `semantic_router.shared_logits`; the runtime reports
`full_family_semantic_shared_expert_projection_device_forward_backward: true`.
The semantic-router route ABI now keeps those shared experts always on in the
route tensor. `nfn_native_tile_semantic_shared_topk_route_float32` and its
backward pair write `semantic_shared_experts + top_k` route slots, with shared
experts first and selected semantic/free experts after them; downstream MoE
dispatch, chunk broadcast, and route-gradient aggregation consume that same
width.
`nfn_native_tile_semantic_shared_forced_topk_route_float32` adds the canonical
per-term forced-target selector: if the semantic target matrix marks any valid
dimension for a row, dynamic top-k selection is restricted to those semantic
dimension experts, with shared experts still prepended to the route tensor.
Semantic-router expert combine uses the full MoE route-broadcast and weighted
selected-expert combine path and reports
`full_family_semantic_expert_combine_device_forward_backward: true`.
Canonical 86-d semantic-family production runs also expose
`semantic_projector.topic_heads.weight`, a packed per-term topic-head parameter
matching the shipped `vocab_86d_o200k.json` term-count layout, plus
`semantic_projector.sig_head.weight` and persistent non-AdamW
`semantic_projector.residual_head.{0,2}.weight` buffers. The route-time native
path computes that residual MLP forward and reports
`full_family_semantic_chunk_projector_residual_device_forward: true`; the raw ABI
functions `nfn_native_tile_semantic_target_matrix_from_tokens_u16_int64` and
`nfn_native_tile_semantic_alignment_packed_loss_backward_float32` derive full
semantic target matrices from token-shard batches and train the packed per-term
head with device-side semantic-alignment loss/backward. Semantic JEPA
target-topic route distillation uses
`nfn_native_tile_semantic_target_topic_packed_distillation_backward_float32` in
the same canonical mode so the detached teacher scores come from per-dimension
term softmax confidences rather than compact 86-wide logits. Runtime JSON reports
...
`full_family_semantic_target_matrix_device_token_derivation: true`. Semantic
JEPA also exposes explicit `online_chunk_projector.*` and frozen
`target_chunk_projector.*` topic/signature/residual parameter names for the
template chunk-projector surface and reports
...
`architecture_parameter_layout.torch_state_dict_aliases` arrays also map the
packed native topic-head buffers to identical Torch `topic_heads.N.weight`
aliases and report
`full_family_semantic_chunk_projector_modulelist_state_dict_aliases: true`;
the older route-time `semantic_projector.*` buffers remain present for
compatibility and non-JEPA semantic routing. In that
canonical mode the legacy compact 86-wide topic head remains allocated only for
noncanonical fallback compatibility and is not used for forward, backward, or
gradient collection.
The native full-family semantic JEPA contract also reports
...
consumed semantic MoE-JEPA production graph surfaces. The remaining full-step
readiness gap is CUDA graph capture, not semantic projector shape coverage.
The raw native Tile ABI includes
`nfn_native_tile_semantic_packed_topic_to_padded_float32` for canonical
semantic-router production steps. It converts packed per-term topic logits into
a padded `[rows, semantic_vocab_dims, max_terms]` float32 surface and writes
zeroes for invalid padded term slots. Native route scoring, semantic-vector
argmax, alignment, and distillation continue to use packed logits, but
plan/runtime JSON reports
`full_family_semantic_chunk_projector_padded_topic_logits_device_forward: true`
when the padded inspection surface is materialized after route-time,
target-topic, masked-online, and target projector per-term projections.
Canonical semantic-router route policy uses
`nfn_native_tile_semantic_route_policy_packed_topic_float32` to add the
Torch-router-style per-dimension max-softmax topic confidence scores from the
packed per-term topic head before top-k selection, and reports
`full_family_semantic_route_policy_packed_topic_scores_device_forward: true`.
The canonical path also exposes
`nfn_native_tile_semantic_route_policy_packed_topic_matrix_float32`, which boosts
every valid per-dimension target from the device semantic target matrix before
top-k selection and reports
`full_family_semantic_route_policy_target_matrix_device_forward: true`.
Canonical semantic-router route hashing also exposes
`nfn_native_tile_semantic_vec_from_packed_topic_float32`, which converts packed
per-term topic logits into per-dimension argmax semantic-vector coordinates for
the hash, shared-expert, and free-expert route-bias paths. Plan/runtime JSON
reports
...
true`. The raw ABI also exposes
`nfn_native_tile_semantic_signature_scalar_float32`, which reduces semantic
projector signature logits to the 0..1 scalar coordinate; semantic-router JEPA
uses it for masked-online and target projector signature forward coverage. The
native semantic-router JEPA path also runs the semantic projector residual MLP
with the existing Tile Linear/GELU composition over the online and target
`jepa_*_encoder.semantic_projector.residual_head.{0,2}.weight` surfaces.
Plan/runtime JSON reports
...
The residual projector output is currently unconsumed by the semantic encoder
graph, so native family parameter stores persist
the semantic projector residual-head weights but keep those buffers out of AdamW
until a real gradient path exists. Runtime JSON
reports
`full_family_semantic_masked_online_projector_residual_adamw_skip_exact: true`
for that unused-parameter parity.
The same semantic-router JEPA path replays the masked-online hidden backbone
before backward and collects
`jepa_online_encoder.backbone.final_norm.weight` plus every layer's FFN norm,
router, expert, attention norm, Q/K/V projection, and attention-output
gradients, reporting
...
true`. Checkpoint/layout metadata also reports
`full_family_semantic_jepa_encoder_state_dict_aliases: true` and maps native
semantic JEPA encoder buffers to Torch `online_encoder.*` and
`target_encoder.*` aliases, including split MoE `dispatch.w1`,
`dispatch.w2`, and `dispatch.w3` views over the packed expert tensors. The
consumed chunk-projector and masked-online encoder exact-parity flags now report
true; full-step CUDA graph capture remains separately guarded. The semantic JEPA loss path now materializes
online and target packed topic vectors from
...
chunk-projector `sig_head.weight`,
and reports
`full_family_semantic_jepa_topic_vector_predictor_device_forward_backward:
true` plus `full_family_semantic_jepa_signature_coordinate_backward: true`.
The same raw ABI exposes `nfn_native_tile_uint16_to_int64` and
`nfn_native_tile_uint8_to_int64` for device-side token/byte widening before LM
losses. Full-family token trainers use the uint16 form, and HNet byte-LM uses
the uint8 form so raw byte shard batches do not need host int64 staging.
Diffusion trainers use `nfn_native_tile_diffusion_mask_u16_int64` to convert
sampler-owned uint16 tokens into deterministic masked uint16 inputs plus int64
denoising targets on device.
Non-semantic JEPA family trainers use
`nfn_native_tile_native_family_jepa_mask_u16_float32` to materialize deterministic
random/block masked uint16 decoder tokens and matching float32 mask weights in
one device pass before token embedding. Semantic JEPA uses the same U16 mask ABI
to materialize masked online token IDs. Canonical semantic MoE-JEPA routes those
tokens through `jepa_online_encoder.backbone.*` and no longer persists the legacy
...
migrate older canonical semantic MoE-JEPA native checkpoints before resume.
The compact native JEPA target projection uses a separate original-token
embedding buffer and accumulates that target-branch embedding gradient against
the original uint16 token IDs.
Universal trainers use `nfn_native_tile_act_pack_step_float32`,
`nfn_native_tile_act_prepare_weights_float32`, and
`nfn_native_tile_act_unpack_step_grad_float32` to keep ACT recurrent-state
stacking, halt-weight/remainder preparation, and ACT-gradient fanout on device.
Semantic family trainers use
`nfn_native_tile_semantic_targets_from_tokens_u16_int64` to materialize per-row
semantic target IDs and validity bytes directly from the staged uint16 token
batch inside the production step. The older
`nfn_native_tile_semantic_targets_from_matrix_int64` matrix path remains for
diagnostic callers that explicitly provide host semantic-target rows, but native
semantic dataset loops keep their hot production path token-only.
The family parameter store also uses ordered `cudaMemcpyAsync` uploads when
the runtime provides that symbol, which is required for capture-compatible
batch and constant staging.
...
density/loss, and the combined weighted objective. Native MoE-JEPA parameter
stores also carry the frozen `target_encoder.backbone.*` router/expert stack and
`target_encoder.projector.*` MLP, and the production path forwards original
tokens through that frozen target branch as a stop-gradient path:
...
nfn-native-train --base-model dense-jepa-evo \
  ...
  --native-cuda-tile-ops-lib PATH
nfn_jepa_native_train --smoke-dense-jepa-full-loop-step --tile-ops-lib PATH
nfn-native-train --base-model dense-jepa-evo \
...
that launches semantic planner forward/backward, semantic hash, alignment loss
items, device-side loss/count reduction, and AdamW:
...
softmax-distillation partials, semantic route-distillation backward, semantic
target-topic route-distillation backward, attentionless decoder projection,
expert bias add, route-balance density/loss, latent MSE, and AdamW.
...
Full-family LLaMA-derived native production steps also require
`nfn_native_tile_sum_accumulate_float32` for LM-head loss reporting. The chunk CE
partials are first reduced with `nfn_native_tile_sum_partials_float32`, then each
reduced chunk contributes to a device scalar through `sum_accumulate`; the host
readback is packed with the other objective totals into one scalar-vector D2H
copy after reporting totals are ready. Plan and runtime JSON report
`full_family_lm_loss_device_accumulation_before_reporting` and
`full_family_lm_loss_reporting_readback_after_lm_backward` plus
`full_family_loss_reporting_single_scalar_vector_readback` while
`full_step_cuda_graph_capture_ready` remains false.
Semantic-router production steps use the same sum-accumulate ABI for semantic
route and target-topic distillation reporting losses, accumulating per-layer
reduced losses into a device scalar before the packed reporting readback.
JEPA MSE, compact semantic CE, and per-term semantic alignment loss/count
reporting totals use the same device-scalar route before the packed final
readback.
...
Long native training keeps LM-head graph setup prewarm on by default for
throughput; set `NFN_NATIVE_GPT_LM_HEAD_COOPERATIVE_GRAPH_PREWARM=0` or
`NFN_NATIVE_GPT2_LM_HEAD_COOPERATIVE_GRAPH_PREWARM=0` only for startup
diagnostics. `NFN_SM120_NATIVE_CANDIDATE_PROFILE=lm_head_graph_prewarm` now keeps
...
ThunderKittens headers and defines `LLMK_SM120_USE_CUBLASLT_GEMM` so the
restored TK attention build stays on the supported cuBLASLt GEMM route for the
trainer-facing shared library.
...
from neuralfn.native_gpt import build_native_gpt_compiled_cli_run_config, build_native_gpt_run_config, capture_native_gpt, run_native_gpt
...
print(config.compiled_cli_command())
run_native_gpt(config)
captured = capture_native_gpt(config)
print(captured.stdout)
```

`build_native_gpt_run_config()` resolves `fineweb_train_000000.bin` plus `fineweb_val_000000.bin` directly when the dataset is already cached as matching uint16 shards. That fast path avoids importing `server.dataset_manager`, NumPy, tiktoken, or Torch and does not scan the full dataset to estimate a schedule before launch. `build_native_gpt_compiled_cli_run_config()` is even lighter for the no-Python cached-shard CLI: it accepts a dataset alias/path and leaves shard metadata validation to the compiled C++ resolver, so Python does not need `meta.json` before handoff. The compiled resolver also accepts llm.kittens-style `TinyStories_train.bin` / `TinyStories_val.bin`; `--tinystories` resolves to `/mnt/disk2/dev/open-source/llm.kittens/dev/data/tinystories` when present, `NFN_LLM_KITTENS_TINYSTORIES_DIR` overrides that directory, and a direct `TinyStories_train.bin` path infers the sibling validation file. Raw-text datasets still lazy-load the dataset manager only when the uint16 cache must be materialized. Tokenizers whose ids do not fit in `uint16`, such as `o200k_base`, are rejected for this path. `NativeGptRunConfig.compiled_cli_argv()` wraps the Tile-CUDA training command through the no-Python cached-shard CLI. Use `capture_native_gpt(config, runner="auto")` or compatibility `capture_native_gpt2(config, runner="auto")` when an SDK caller needs captured JSON/stdout/stderr from a full config; the auto runner prefers the C++ binding capture path and falls back to the compiled CLI or launcher only when requested or when the binding is unavailable. The generic module exports `NativeGptRunConfig`, `NativeGptRunnerStatus`, and `NativeGptCheckpointInfo` as GPT-native dataclass subclasses, plus `native_gpt_activation()`, `native_gpt_kernel_backend()`, `normalize_native_gpt_encoding_name()`, and `native_gpt_encoding_vocab_size()`, so new SDK code does not need GPT-2-named helper imports. Build the in-process compatibility SDK binding with `bash tools/build_native_gpt2_binding.sh`, the launcher with `bash tools/build_native_gpt2_launcher.sh`, the no-Python cached-shard CLI with `bash tools/build_native_gpt_cli.sh`, the linked no-Python cached-shard CLI with `bash tools/build_native_gpt_cli_linked.sh`, and the unified no-Python frontend with `bash tools/build_native_train_cli.sh`; the compiled GPT CLI links the shared C++ `token_shards.cpp` resolver for cached-shard validation. `bash tools/build_native_gpt2_all.sh` rebuilds `libnfn_native_train_tile_ops.so` before `nfn_gpt_native_train_linked`, so the linked binary preferred by SDK/CLI startup paths is built against the current Tile kernels. The linked GPT CLI resolves Tile ops ABI symbols from `RTLD_DEFAULT` when invoked with `--tile-ops-lib linked`, avoiding the in-trainer `dlopen` setup cost while keeping the same required-symbol scan. Direct linked no-data preflight smokes, including `--smoke-tile-ops`, `--smoke-nvfp4-pack`, and `--smoke-optimizer-step`, plus the native GPT smoke/training paths that load the raw Tile ABI, use the same `RTLD_DEFAULT` symbol resolution instead of trying to `dlopen` a file named `linked`. `resolve_native_gpt2_cli()` and direct SDK compiled-CLI resolution prefer the dynamic `build/nfn_gpt_native_train`; higher-level CLI startup paths separately prefer a fresh `build/nfn_gpt_native_train_linked` when their linked-binary freshness gate passes and no explicit GPT CLI override is set; GPT2-Evo is a separate preflight-only family target and does not select or exec that dense binary. Use the normal CLI or explicit `--tile-ops-lib PATH` when a dense-GPT benchmark intentionally swaps `libnfn_native_train_tile_ops.so` at runtime. `tools/install_native_gpt2_commands.sh` links `nfn-gpt-native`, `nfn-gpt-native-train`, `nfn-native-train`, `nfn-gpt2-tile-launcher`, and both underscore/hyphen names for built per-family native trainer entrypoints into the active Python scripts directory. `NFN_DATASETS_DIR` overrides the native alias cache root, and `nfn-native-train --base-model gpt --dataset-alias PATH_OR_ALIAS` or `nfn train --base-model gpt --dataset-alias PATH_OR_ALIAS` bypasses Python graph training when shards already exist. The compiled `nfn_native_train` frontend also accepts Python-wrapper aliases directly, including `--dataset tinystories`, `--output`, `--kernel-backend`, `--template` / `--preset`, `--graph`, and `--native-cuda-*`, then injects dense GPT defaults before execing `nfn_gpt_native_train`. `--base-model gpt2`, `--base-model gpt3`, and `--base-model nanogpt` are dense GPT aliases for the same native kernel family; GPT3 defaults to `--train-seq-len 2048` when selected by base model or `--template-name gpt3`, unless a custom graph or explicit sequence length is supplied. The implicit GPT3 batch size is 32 to preserve the 65,536-token microbatch unless `--batch-size` is explicit. NanoGPT defaults add `--template-name nanogpt`, and the shared dense loop uses the selected 320-wide/5-head/5-layer geometry for full-transformer training. Otherwise `--template-name` and `--graph-file` are the architecture source of truth, and compiled plan/runtime JSON preserves selected dense GPT `model_family` labels while reporting `architecture_source`, `architecture_contract`, `model_family_context_policy`, and `resolved_native_template_name` for that decision. The default public template is `template_name="gpt"`, which currently resolves to the implemented dense GPT native topology. The compatibility `NativeGpt2RunConfig` and `build_native_gpt2_*` helpers now canonicalize dense GPT selectors to `model_family="gpt"`; pass `template_name` or `graph_file` for architecture selection instead of keying off the model-family label. The shared C++ token sampler reads contiguous shard segments for each batch instead of opening a shard per sequence chunk; token-shard JSON reports `batch_read_strategy: "contiguous_shard_segments"`. `nfn-native-train --list-models --json` reports the compiled native training registry; dense GPT defaults to the NeuralFn-owned Tile-CUDA `--train-transformer-lm` loop and its JSON `block_state_layout` exposes block-vector allocation/init/zero/clip/AdamW/checkpoint/tape/forward/backward loop flags. NanoGPT is reported as an implemented dense GPT selector; explicit `--train-token-lm` remains available on `nfn_nanogpt_native_train` for token-LM diagnostics. Canonical LLaMA reports `native-trainer-covered` / `native-loop-covered` and its production full-geometry dataset loop; JEPA, semantic/MoE, and DeepSeek entries report their current per-family coverage, while GPT2-Evo reports `preflight-only` with graph-faithful whole-block evolution blocked before delegate exec. The token-only NanoGPT C++ target remains available for explicit diagnostics: `nfn_nanogpt_native_train --print-plan` emits a JSON training contract for shape, schedule, AdamW settings, token-shard constraints, contiguous parameter/gradient/AdamW-state buffers, AdamW decay/no-decay groups, forward/backward/optimizer `training_step_plan`, and required CUDA Tile kernels without importing Python or Torch. `nfn_nanogpt_native_train --train-token-lm --tile-ops-lib PATH --dataset-alias PATH_OR_ALIAS --max-steps N` runs the tied token-embedding LM as a real multi-step native loop over cached shards and records validation losses from validation shards according to `--eval-every-steps`, `--eval-batches`, and `--eval-batch-size`. `nfn_nanogpt_native_train --check-tile-ops --tile-ops-lib PATH` loads `libnfn_native_train_tile_ops.so` with `dlopen` and verifies every NanoGPT-required raw ABI symbol from the compiled binary. Tied LM head input/weight backward is represented through the raw linear backward ABI in that plan. It defaults to `dropout_p=0.0`; nonzero `--dropout-p` verifies the deterministic inverted-dropout Tile ABI through `nfn_native_tile_dropout_forward_float32` and `nfn_native_tile_dropout_backward_float32`. `NFN_NATIVE_GPT2_BIN_DIR` overrides where command symlinks are installed, `NFN_NATIVE_TRAIN_CLI` overrides the unified frontend path, `NFN_NATIVE_<MODEL>_CLI` overrides a per-family native trainer such as `NFN_NATIVE_NANOGPT_CLI`, `NFN_NATIVE_GPT_CLI` overrides the compiled GPT CLI path, and `NFN_NATIVE_GPT_LAUNCHER` overrides the launcher path. `NFN_NATIVE_GPT2_LAUNCHER` remains a compatibility fallback when the generic launcher override is unset. `NativeGptRunConfig`, `NativeGpt2RunConfig`, and native checkpoint sampling default `cuda_visible_devices="0"` and `cuda_device_max_connections="1"` before launching native code; existing `CUDA_VISIBLE_DEVICES` or `CUDA_DEVICE_MAX_CONNECTIONS` environment variables still take precedence. SDK compiled native GPT launches default `NFN_NATIVE_GPT_DEFER_PREWARM_AFTER_STEPS=0`, unless a caller already set a prewarm-threshold env var, so multi-step runs keep throughput QKV/LM-head prewarms on for model-quality training. Set a positive defer threshold only when intentionally selecting the benchmark deferred-prewarm diagnostic route. Set `cuda_visible_devices="dedicated"` only when a run should opt into the `nvidia-smi` display-disabled GPU selector. The top-level `neuralfn` package and native cached-shard path avoid importing Torch; `cli/scripts/train_gpt.py` is the canonical native-only GPT script, while `cli/scripts/train_gpt2.py` remains a compatibility entrypoint. Direct execution with the default `compiled-cli` runner reaches the compiled C++ CLI before importing `train_gpt_native.py`. Default GPT `nfn train` commands hand off to the compiled Tile-CUDA frontend before graph-backed Python imports. The CLI wrapper defaults to `--native-cuda-runner compiled-cli`; set `smoke_nvfp4_pack=True` on `NativeGptRunConfig` or `build_native_gpt_compiled_cli_run_config()` to emit `--smoke-nvfp4-pack` for the native C++ NVFP4 pack/dequantize preflight. Call `run_native_gpt(..., runner="auto")` or pass another runner explicitly when you want SDK binding or launcher behavior.

The public `NativeGptRunConfig` / `NativeGpt2RunConfig` dataclasses and their
builders carry `lr_schedule`, `lr_schedule_total_steps`, `train_seed`,
`resume_from_checkpoint`, `graph_fingerprint`, and `graph_preflight_proof` in addition to the existing
`graph_file`. `train_seed` is distinct from the generation sampler's `seed`.
The builders accept optional `final_lr_fraction`; an explicit value takes
precedence over the fraction derived from `min_lr`. Schedule, final-LR, and
train-loss aliases are normalized before native quality defaults, and
`compiled_cli_argv()` forwards all of these fields through the direct GPT,
unified native, and SM120 frontends. The legacy llm.kittens short argv cannot
represent a non-default schedule or strict continuation/graph fields:
`argv()`/`command()` reject rather than discard them, serialized configs retain
`legacy_command_error`, `prefer_compiled_cli`, and compiled-CLI/Tile-launcher
forms, and dataset-alias or strict
Tile binding resolution prefers the compiled argv. A hand-built config cannot
pair these native-only fields with a legacy short executable and omit
`dataset_alias`; use a compiled-CLI builder or identify the dataset alias and
compiled trainer. Hand-built non-Tile configs reject these fields as well.
For an explicit `template_name="gpt2_diff"`, `graph_file`,
`graph_fingerprint`, and `graph_preflight_proof` must be supplied together. The
digest must already be exactly 64 lowercase hexadecimal characters and neither
digest nor proof path may contain surrounding whitespace. All three may remain
absent only while constructing selector/catalog commands; normal low-level
differential training rejects the unbound selector before Tile/CUDA work. Use
the proof emitted by trusted planner materialization; its unkeyed digest is
local-handoff integrity, not caller authenticity.

Low-level packed `gpt2_diff` checkpoints use
`neuralfn.native_gpt2_diff.training_checkpoint` version 2 while retaining
`checkpoint_kind=trained_dense_v5_plus_diff_v1` for the unchanged five binary
formats. Continuation preflight runs before Tile/CUDA/H2D and binds the graph;
DONE-gated binaries; optimizer/microbatch and sampler counters; seed;
batch/accumulation shape; the resolver-ordered training shard basenames,
byte/header/token metadata and contents; optimizer/LR horizon and LM-head chunk;
effective BF16 routes; and a canonical profile of supported effective
arithmetic, precision, and reduction routes. Stable no-follow descriptors remain open for training reads;
validation shards are outside the identity. `--max-steps` adds work, omitted
resume horizon inherits version-2 metadata, and explicit horizon or seed
mismatch rejects. Final export is exclusive/create-only, fsyncs files and the
directory, and creates/fsyncs DONE last with rollback of newly created files on
in-process failure. It is not atomic-rename publication, ancestor symlinks are
not rejected, and metadata-smoke output is outside the transaction. The learned
trainer requires the learned-lambda ABI; the legacy fixed-lambda ABI remains
outside this contract with rounded-output/non-layer-local backward debt. Raw
learned-ABI callers must keep one CUDA device current: workspace ownership is
currently keyed by stream rather than device plus stream. The shipped trainer
uses one selected device; multi-device raw-ABI ownership is not yet supported.

The native full-geometry LLaMA family trainer exposes three production
quantized linear contracts through the raw Tile ABI. `ternary_b158` uses
per-output-row ternary dequantization, `fp8_llama` and `deepseek_v4` use FP8
E4M3 for native linear projections, and `mxfp4_llama` uses 32-value MXFP4 E2M1
blocks. Forward and input-backward
share the same dequantized values, while weight gradients remain float32
straight-through updates. Family plan JSON reports the selected contract as
`architecture.linear_quantization`; this is separate from the PyTorch
`runtime="megakernel"` setting.
DeepSeek-V4 additionally reports
`architecture.expert_quantization: "mxfp4_e2m1_block32"` and uses
`nfn_native_tile_moe_swiglu_forward_quantized_float32` /
`nfn_native_tile_moe_swiglu_backward_quantized_float32` for expert SwiGLU
forward/backward with MXFP4 expert-weight reads and straight-through float32
expert gradients. It also reports
`architecture.router_score_fn: "sqrt_softplus"` and binds
`nfn_native_tile_topk_route_sqrt_softplus_float32` /
`nfn_native_tile_topk_route_sqrt_softplus_backward_float32` for selected-route
forward/backward.

The native `diff_transformer` family path uses the Tile split/merge-last-dim
and differential combine/backward ABI to execute two causal attention branches
over split query/key channels. A persistent `attention.diff_lambda` scalar is
read from the device parameter store and its gradient is accumulated into the
shared AdamW update; the plan contract reports
`attention_variant="differential"`.

The native `kv_pca_llama` path uses the existing Tile linear ABI for shared
per-head K/V PCA encode and decode stages around attention. It persists the
four matrices per layer and runs their input/weight backward paths before the
usual Q/K/V projection gradients; `kv_pca_compressed_dim` defaults to one
quarter of the attention head dimension.
...
`--evo-layer-mutation-scale`. They now select a fail-closed diagnostic contract,
not an executable optimizer-loop implementation. The authored GPT2-Evo graph
excludes every tensor in the designated transformer block from gradient/AdamW
optimization and evolves the whole block; the retained generic implementation
would instead AdamW-update the block and evolve only `block_N.ln1.weight`.
Normal/startup/print-command requests therefore return status 2 before Tile
load, CUDA setup/allocation, output creation, or model mutation. Print-plan and
dry-run emit `native-preflight-blocked-graph-faithful-layer-evo-missing`, exact
whole-block missing gates, and all side-effect-attempt fields false. A final
`--no-layer-evo` preserves ordinary dense training. Explicit kernel,
checkpoint, and metadata smokes remain isolated diagnostics; the raw
`nfn_native_tile_evo_mutate_candidates_float32`,
`nfn_native_tile_evo_select_best_loss_float32`, and
`nfn_native_tile_evo_adopt_candidate_float32` primitives do not prove a
graph-faithful optimizer/checkpoint path.
...
in GPT2-Evo preflight metadata, but `tile_cuda.effective_activation_dtype`,
...
nodes or Torch.
The compiled per-family binaries for LLaMA, MixLLaMA, JEPA,
semantic-router MoE, DeepSeek-V4, and Jamba expose the same native boundary as
...
missing, and present Tile ABI symbols. Canonical LLaMA now reports
`status: "native-trainer-covered"`, `trainer_loop_status:
"native-loop-covered"`, `production_training_loop: true`, and full architecture
optimizer/persistence coverage; its exact graph-authored adapter is
execution-ready after planner validation. Registry/family-binary reachability
is distinct from graph-file readiness: seven reviewed GPT-2 profiles plus
canonical `llama`, its exact compile-runtime alias `llama_fast`, and exact
standard-MoE `moe`/`mixllama`/`mixllama_fast`, plus trusted-planner proof-bound
`gpt2_diff` training, make 13 execution-ready graph adapters. `gpt2_diff` stays
closed at the Native IR migration/resident boundary. Its low-level packed trainer now learns and
AdamW-updates one FP32 lambda per layer and persists it in additive graph-bound
differential sidecars while leaving dense-v5 unchanged. It fails before Tile
load/state mutation unless packed QKV, sequence length at least 16,
divisible/even head geometry, BF16 QKV-gradient handoff, and all three learned-
lambda symbols
...
are available. The backward symbol recomputes each layer's two attention branches,
backpropagates the head-wise RMSNorm, and accumulates the FP32 scalar gradient
through its `grad_lambda` pointer. Completed checkpoints add
`diff_parameters_XXXXXXXX.bin`, `diff_optimizer_XXXXXXXX.bin`, and strict
`model_XXXXXXXX.diff.json` before the ordinary empty DONE marker. Migration and resident
inference do not consume that state yet. Both LLaMA profiles use the canonical native `llama` ABI
while retaining source-profile provenance. LLaMA smoke commands remain
available as explicit diagnostics, while incomplete neighboring families keep
their current missing requirements visible.
MoE-family
...
--tile-ops-lib PATH` or the unified
`--native-cuda-smoke-moe-transformer-lm-train-step` alias on standard MoE
...
`nfn_seq2seq_native_train --smoke-seq2seq-cross-attention-step --tile-ops-lib
PATH` or the unified `--native-cuda-smoke-seq2seq-cross-attention-step` alias
...
`nfn_seq2seq_native_train --smoke-seq2seq-loss-composition-step --tile-ops-lib
PATH` or the unified `--native-cuda-smoke-seq2seq-loss-composition-step` alias
...
backward, and AdamW as a raw CUDA Tile train-step slice. Use
`nfn_hnet_lm_native_train --smoke-hnet-byte-patch-backward-step --tile-ops-lib
...
current linked-trainer 49152-row rerun changed only the prewarm row strategy and
kept hot route counters unchanged, but regressed train-loop wall to
`1.001124x`, first-step CUDA-event timing to `1.003118x`, forward-QKV
first-step timing to `1.075812x`, setup wall to `1.026683x`,
startup-plus-train-loop to `1.003148x`, and candidate-over-llm.kittens
train-loop wall to `1.010542x`.
Use `NFN_SM120_NATIVE_CANDIDATE_PROFILE=long_run_qkv_forward_prewarm` when the
question is specifically the explicit long-run deferred-prewarm benchmark path. It forces full-row
...
passed candidate-over-llm.kittens train-loop wall at `0.998803x`. It remains
rejected because the strict reference first-step gate missed at `1.023625x` and
...
test the rejected opt-in overlap variant against the benchmark long-run deferred
...
`NativeGpt2RunConfig.train_loss_every_steps` default to `250` and forward
`--train-loss-every-steps` through `compiled_cli_argv()`. Set it to another positive value to change sampled native training-loss cadence while keeping
validation loss on `eval_every_steps`, or set it to `0` for timing-only runs. The compiled transformer-LM loop collects
...
tokens/sec to `0.997262x`, so the default remains the cooperative
sequence-wrapper LM-head route.
...
The aggregate `linear_bf16_a_pack_count` remains for continuity; the split
counters identify cached A/B operand packs separately from transient A/B
activation packs.
...
selectors use the 320-wide/5-layer template geometry. That NanoGPT mapping is
dispatch/geometry coverage, not graph-faithful persistence or inference: its
bias-free/dropout graph remains blocked against the biased, dropout-free
dense-v5 contract. Structurally different
...
The `gpt2_moa` selector is implemented in the compiled transformer-LM loop, not
just in selector metadata. It probes the shared MLP backbone with GELU, ReLU,
SiLU, and ReLU2 at step 1 and every `moa_interval` steps, selects the lowest
current-batch loss, and uses that activation for the following backward/update.
Runtime JSON exposes `native_moa_enabled`, `moa_selected_activation`,
`moa_probe_runs`, `moa_candidate_evals`, and `moa_last_probe_losses`.
For graph-authored production output, the trainer writes source-bound
`model_XXXXXXXX.moa.json` beside the dense-v5 `.bin` and empty DONE marker. The
metadata binds the exact graph/model, canonical candidate order, final selected
activation, and positive interval; migration and resident inference must use
that JSON rather than treating the bare dense-v5 file as MoA-complete. Resume
validates the same sidecar and restores its activation without a new candidate
probe; missing or tampered metadata fails closed. Direct selector-only
`gpt2_moa` first-leg training remains supported and writes ordinary dense-v5;
only its unbound resume is rejected because no selection contract exists.

GPT-2 evo's family-specific C++ binary exposes the same selector fields and
catalog for SDK/subprocess callers. It is currently a **preflight-only** target:
dense GPT-2-compatible templates, including `gpt2_modern`, report
...
`selected_graph_native_runnable: false`, and
`training_execution_blocked: true`. The authored graph excludes every tensor in
the designated block from gradient/AdamW updates and evolves that whole block;
the current dense path AdamW-updates the block and evolves only
`block_N.ln1.weight`. `missing_graph_faithful_gates` lists whole-block optimizer
exclusion, whole-block mutation/evaluation/selection/adoption, and checkpoint/
resume parity. Structurally different shipped templates, custom graph files,
and typoed template names also report non-runnable support statuses without
importing the graph-backed runtime.
...
graph-editor nodes. This isolated primitive diagnostic repeats the non-runnable
status and missing gates; it is not training evidence. `--print-plan` and
`--dry-run` inspect the blocked plan. Normal execution, `--startup-only`, and
`--print-command` return 2 before delegate exec, CUDA setup/allocation, or
output creation. `--no-layer-evo` does not convert this entrypoint into a
GPT2-Evo trainer; it directs callers to `nfn_gpt_native_train` for the distinct
ordinary dense workflow.
...
Compiled native family dataset loops use the same accumulation invariant even
when they report family-specific counters instead of dense GPT
`train_microbatches_completed`. `steps_completed` is the optimizer-step count;
`train_batches_sampled` advances once for each accumulated train microbatch.
For the default family shape, DeepSeek V4, JEPA Evo, semantic/MoE, LLaMA,
Jamba, Seq2Seq, Diffusion, TTT, HNet, and universal-transformer loops should
therefore sample eight train batches before each reported optimizer step
completes.
...
is already present, so short same-script runs can isolate the
deferred-prewarm diagnostic branch. Real native training defaults
`NFN_NATIVE_GPT_DEFER_PREWARM_AFTER_STEPS=0` and keeps throughput prewarms on
unless the caller explicitly selects deferred prewarm. In benchmark
deferred-prewarm mode the parity wrapper gates steady-state CUDA-event step time
by default instead of counting the expected first-step deferred-prewarm cost as a
train-loop regression. Set
...
For long native GPT quality runs, the same native trainer keeps throughput
prewarms on by default. Set `NFN_NATIVE_GPT_DEFER_PREWARM_AFTER_STEPS` to a
positive threshold (`NFN_NATIVE_GPT2_DEFER_PREWARM_AFTER_STEPS` /
`NFN_TILE_CUDA_DEFER_PREWARM_AFTER_STEPS`) only when intentionally benchmarking
the deferred-prewarm route. Benchmark that route with
...
60 warmup pairs for this profile, or for a measured benchmark-only
...
for benchmark-only deferred-prewarm current-vs-llm.kittens checks; set
...
defaults as `cli/scripts/train_gpt.py`: validation every 5000 optimizer steps
over 20 batches, sample cadence, train-loss cadence, checkpoint cadence, `64 x 1024 -> 524288` token
...
the GPT2-Evo preflight-only target, NanoGPT native preflight, and placeholder
...
SDK C++ bindings reject Python/shell launchers (`python`, `bash`,
`/usr/bin/env`, `*.py`, `*.sh`) on the native training path. Set
...
`graph_file=` for reviewed dense GPT adapters. The resolved compiled command
appends `--template-name` and `--graph-file` only when those flags are not
already present in raw `args`, preserving explicit CLI overrides while making
GPT preset and native-compatible custom graph selection first-class in the SDK.
`NativeTrainRunConfig` additionally supports the exact canonical LLaMA graph
adapter, but only through a plan-backed handoff: call
`plan_native_graph_training(graph_file)`, use `model_family="llama"`, pass the
same path via `graph_file`, and pass the returned `trainer_arguments` unchanged.
The SDK replans the graph and rejects path overrides, conflicting templates,
fabricated fingerprints, missing graph-owned arguments, and duplicate or
different graph-owned values before resolving the family binary.
...
BF16/BF16 QKV dWeight+bias accumulation is default-on.
...
`ar-plus-jepa-loss-composition-smoke`; dense JEPA, MoE-JEPA, and
semantic-router JEPA report frozen target-backbone exact parity once the
corresponding target backbone/projector path is active. Semantic projector
exact-parity remains tracked through semantic-specific native-loop fields.
