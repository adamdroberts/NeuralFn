...
_DEFAULT_NATIVE_GPT2_TARGET = "/mnt/disk2/dev/open-source/llm.kittens/train_gpt2cu"
...
def _native_target_path() -> str:
    ...
    if requested:
        return requested
    default_path = Path(_DEFAULT_NATIVE_GPT2_TARGET)
...
def _fast_compiled_cli_argv(argv: list[str]) -> list[str] | None:
    runner = (_arg_value(argv, "--native-cuda-runner") or os.environ.get("NFN_NATIVE_GPT2_RUNNER", "compiled-cli"))
...
from train_gpt2_native import NATIVE_GPT2_DEFAULTS, build_parser, main  # noqa: E402
...
def resolve_mode_defaults(args):
    if not str(getattr(args, "output", "") or "").strip():
        args.output = GPT2_DEFAULTS["output"]
    return args
