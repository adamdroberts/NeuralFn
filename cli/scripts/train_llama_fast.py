...
if __name__ == "__main__":
    reject_torch_training_by_default("train_llama_fast.py", native_target="nfn train --base-model llama", model_family="llama")
...
from neuralfn.config import build_llama_fast_megakernel_spec, build_llama_fast_spec, model_spec_to_dict
...
LOGGER = logging.getLogger("llama_fast_harness")
...
def mode_name(*, megakernel: bool) -> str:
    return "llama_fast_megakernel" if megakernel else "llama_fast"
...
def build_parser() -> argparse.ArgumentParser:
    parser = create_argument_parser(description="Train llama_fast with the NeuralFn CUDA harness.")
    parser.add_argument("--megakernel", action="store_true", help="Use the llama_fast_megakernel preset/runtime.")
    parser.add_argument("--run-id", default=env_str("RUN_ID", LLAMA_DEFAULTS["run_id"]))
    ...
    add_dataset_selector_arguments(
        parser,
        default_alias=env_str("DATASET_ALIAS", LLAMA_DEFAULTS["dataset_alias"]),
    )
...
def build_graph(args: argparse.Namespace, dataset_name: str):
    builder = build_llama_fast_megakernel_spec if args.megakernel else build_llama_fast_spec
    spec = builder(
    ...
    *,
    ...
    def on_step(info: dict[str, Any]) -> None:
        ...
        log_stage(
            ...
            f"loss={float(info.get('loss', float('nan'))):.6f} "
            f"elapsed={elapsed}{lr_text}{grad_accum_suffix}"
        )
