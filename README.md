...
steady-state gate: NeuralFn over llm.kittens measured `0.999098x` steady-state
CUDA-event step time and `1.001006x` steady-state tokens/sec, with zero external
compute processes on the selected GPU. The full 3-step train-loop wall ratio was
`1.034260x` and first-step CUDA-event ratio was `1.103108x`, which is the
