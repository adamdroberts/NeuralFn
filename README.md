...
Standard MoE-family preflights now require the raw Tile ABI for routed expert SwiGLU forward/backward as `nfn_native_tile_moe_swiglu_forward_float32`/`nfn_native_tile_moe_swiglu_backward_float32`, in addition to top-k routing, expert-route broadcast, route-balance loss, and AdamW. Semantic-router MoE preflight also requires semantic hash routing, chunk-route broadcast, semantic-alignment item losses, attentionless decoding, expert bias add, latent MSE, and the same routed expert forward/backward ABI. `required-tile-symbols-present` means the loaded Tile ops library exports that MoE/semantic/JEPA kernel surface; `family-native-loop-missing` still means the model-specific optimizer loop, checkpoint, and inference wiring has not landed yet.
