...
`NFN_NATIVE_GPT_BF16_ATTENTION_GRAD_OUT=1` is an opt-in packed-attention profiling switch. It makes the attention projection dInput GEMM write BF16 grad-out bits directly and feeds those bits into the packed attention backward bridge. Runtime JSON reports `attention_backward_bf16_grad_out_handoff_enabled`, `attention_backward_grad_out_dtype`, BF16 scratch sizes, and the updated QKV bridge strategy. Leave it disabled for normal training: the CUDA 13.3.33 dedicated RTX 5090 2026-06-25 3-step, 2-sample recheck improved attention dprep timing to `0.802066x` and steady-state CUDA-event mean to `0.996857x`, but regressed `train_loop_wall_ms_per_step` to `1.004521x`, tokens/sec to `0.995500x`, block backward to `1.010979x`, and MLP projection backward to `1.054337x`.
