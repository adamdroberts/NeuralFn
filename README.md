...
When callers explicitly force lower warmup or step counts while the default long-run deferred-prewarm policy is active, paired metadata now records `default_long_run_defer_prewarm_low_warmup_diagnostic` and/or `default_long_run_defer_prewarm_low_step_diagnostic`. Treat those runs as first-step/prewarm diagnostics; use the default floors or explicit higher values for steady-state parity evidence.
The paired native runtime contract gate now consumes the same trainer fields: long-run deferred-prewarm candidates must report `train_timing_contract: "long-run-deferred-prewarm-steady-state"`, the expected `train_first_step_deferred_prewarm_diagnostic` value for completed steps, and `train_steady_state_parity_metric_available` once more than one optimizer step is measured. A two-step-or-longer measured candidate that lacks the steady-state marker fails the native runtime contract before ratio gates are interpreted.
