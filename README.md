...
coverage remains the default. The matching `store_mlp_blocks3` and
`store_mlp_blocks9` profiles are also rejected: the 2026-06-27 dedicated RTX
5090 gates reduced stored MLP activation bytes further or less aggressively,
but still regressed train-loop wall time, steady-state CUDA-event step time,
tokens/sec, block backward, and MLP projection backward. The matching
