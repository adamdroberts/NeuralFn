...
Native GPT-2 attention now defaults to a packed-QKV SM120 TK bridge on the compiled Tile-CUDA path. The trainer writes the no-bias QKV GEMM output as packed BF16, applies QKV bias in-place, runs packed TK attention directly over the row-major packed QKV tensor, and unpacks only the attention output needed by the projection. Backward uses the matching packed-QKV TK gradient bridge to return row-major `grad_qkv`. Set `NFN_NATIVE_GPT2_PACKED_QKV_ATTENTION=0` only when comparing against the older split-to-heads bridge. Plan and runtime JSON report `packed_qkv_attention_enabled`, packed BF16 scratch bytes, packed QKV layout strategies, and `attention_backward_strategy: "tk-sm120-packed-qkv-bf16-backward-bridge"` when this default path is active.
