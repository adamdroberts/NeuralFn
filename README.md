...
For the composed semantic dense JEPA native train-step slice, use `nfn_semantic_dense_jepa_native_train --train-semantic-dense-jepa-loop-step --tile-ops-lib PATH` or `nfn-native-train --base-model semantic-dense-jepa-evo --native-cuda-train-semantic-dense-jepa-loop-step`; the bare `nfn_semantic_dense_jepa_native_train ...` and `nfn-native-train --base-model semantic-dense-jepa-evo ...` paths also enter this slice by default. It wraps the semantic planner/alignment AdamW train-step through compiled native C++ and reports substep JSON without Torch or graph-editor tensor flow. The payload keeps `production_training_loop: false` and `production-family-forward-backward-optimizer-loop` in `native_training_missing_requirements` until the full dataset loop, checkpoint cadence, and inference metadata path are implemented.
