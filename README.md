...
forward/backward, and the RMSNorm/QKV-projection/packed-attention/residual
forward block slice, while their missing list keeps packed-QKV RoPE attention
block integration, the full LLaMA block forward/backward loop, and family
...
forward/backward loop and family checkpoint/inference wiring. Diffusion
entries now report denoise-head linear/MSE/backward/AdamW plus
timestep/mask/token-CE/backward/AdamW smokes as completed while keeping family
checkpoint/inference wiring in the missing list. Seq2seq entries now
...
composition, and family checkpoint/inference wiring in the missing list. TTT
entries now report the inner linear/MSE/backward/AdamW smoke as completed while
keeping the full inner-update loop and family checkpoint/inference wiring in the
missing list. Universal transformer entries now report recurrent
linear/MSE/backward/AdamW and ACT halt loss/gradient smokes as completed while
keeping family checkpoint/inference wiring in the missing list. HNet entries
...
completed while keeping family checkpoint/inference wiring in the missing list.
...
Jamba layer schedule loop, and family checkpoint/inference wiring in the
missing list.
...
JEPA keeps only family loop/checkpoint/inference wiring in the missing list,
while MoE+JEPA and semantic JEPA still keep router/semantic objective
...
Standard MoE-family preflights now require the raw Tile ABI for routed expert SwiGLU forward/backward as `nfn_native_tile_moe_swiglu_forward_float32`/`nfn_native_tile_moe_swiglu_backward_float32`, in addition to top-k routing, expert-route broadcast, route-balance loss, and AdamW. Semantic-router MoE preflight also requires semantic hash routing, semantic-alignment item losses, route-selection loss partials, softmax-distillation partials, chunk-route broadcast, attentionless decoding, expert bias add, latent MSE, and the same routed expert forward/backward ABI. `required-tile-symbols-present` means the loaded Tile ops library exports that MoE/semantic/JEPA kernel surface; `family-native-loop-missing` still means the model-specific optimizer loop, checkpoint, and inference wiring has not landed yet.
