...
default `50304,49152,768,T,N` row-chunk shape. The default tied LM-head row
chunk is now 49152 rows for the workstation 5090 profile, keeping the default
64x1024 LM-head chunk count at 2 while reserving about 4.95GB of resident
BF16 logit workspace. Pass `--lm-head-row-chunk-size 32768` to reproduce the
previous 5090 default, or pass `--lm-head-row-chunk-size 8192`,
...
3-step, 1-sample parity refresh on the dedicated RTX 5090 measured NeuralFn at
`2512.313 ms/step` versus llm.kittens at `2474.457 ms/step` (`1.015299x`
train-loop wall time, `0.984089x` tokens/sec) with no compute processes on the
selected GPU. The steady-state CUDA-event slice measured `1.011877x`; the
matching 2-step stage-timed attribution run measured `1.008363x` train-loop
wall and `1.015418x` steady-state CUDA-event timing. The remaining gap is still
native GPU kernel throughput, with hot buckets in block backward
(`mlp_proj`, attention SDPA `to_qkv`, QKV, and LM-head backward), not Torch,
Python, startup, graph-editor execution, or external GPU load.
...
old per-chunk LM-head hidden packing route can be retested against the current
full-microbatch BF16 final-norm hidden prepack default without a manual env
pair. The mirror `lm_head_prepack_bf16_hidden_on` profile pins baseline off and
candidate on for explicit rechecks of the promoted default. The CUDA 13.3
dedicated RTX 5090 5-step, 3-sample stage-timed recheck passed the strict gates
at `0.999726x` train-loop wall, `0.997573x` LM-head backward, `0.999748x`
...
`lm_head_logits_bf16_fallback_32768` only for historical diagnostics of the old
32768-row LM-head logits shape with
`NFN_NATIVE_LINEAR_TK_FORWARD_DISABLE_SHAPE=50304,32768,768,T,N`. That profile
is rejected by default under the current 49152-row shape because the CUDA 13.3
dedicated RTX 5090 recheck changed no route counters, strategy strings, linear
shape stats, or cuBLASLt plan cache entries. Use
...
The current-shape fallback is also rejected by default: the CUDA 13.3 dedicated
...
`combined_device_arena` is also rejected by default: a 3-sample real-loop rerun
regressed train-loop wall time to `1.004991x` and tokens/sec to `0.995098x`,
while startup-only setup regressed to `1.063067x`. The trainer now defaults
...
`--smoke-tile-ops` and `--smoke-optimizer-step` also treat `linked` as
`RTLD_DEFAULT` instead of trying to open a file named `linked`, so the linked
binary can verify CUDA fill and AdamW kernels before any dataset is resolved.
...
`NFN_NATIVE_GPT_QKV_DINPUT_BEFORE_DWEIGHT=1` is the matching diagnostic ordering switch for the dense GPT packed-QKV backward path. It runs QKV dInput before QKV dWeight+bias to compare against the reference consumer order, while leaving the default dWeight+bias-first order active. Runtime JSON reports `block_backward_qkv_dinput_before_dweight_enabled` and `block_backward_qkv_dinput_before_dweight_count`; leave it disabled for normal training because the latest dedicated RTX 5090 CUDA 13.3 rerun improved train-loop wall to `0.994580x`, but missed steady-state CUDA-event, LM-head, MLP-projection, and QKV gates. The SM120 candidate wrapper also exposes `qkv_dinput_ln64`, which combines the QKV ordering switch with `NFN_NATIVE_GPT_LAYERNORM_AFFINE_ROW_CHUNK_SIZE=64`; it remains rejected by default after improving train-loop wall to `0.986068x`, steady-state CUDA-event timing to `0.997478x`, total block backward to `0.980591x`, and QKV backward to `0.989746x`, but missing strict LM-head (`1.000400x`) and MLP-projection (`1.001690x`) gates.
...
Use `NFN_SM120_NATIVE_CANDIDATE_PROFILE=linear_bias_row_chunk_256` to recheck the
promoted route against the older 512-row baseline, or `linear_bias_row_chunk_1024`
to inspect the rejected wider chunk. A real 1024-row rerun now requires
`NFN_SM120_NATIVE_ALLOW_REJECTED_CANDIDATE_PROFILE=1`, matching the measured
rejection evidence in the benchmark wrapper. Native training JSON reports
`block_state_layout.linear_backward_bias_row_chunk_size`. The 2026-06-23
dedicated RTX 5090/CUDA 13.3 same-script gate measured the 256-row route at
`0.995306x` train-loop wall time, `1.005073x` train tokens/sec, and `0.994292x`
total wall time across three five-step samples. The 1024-row route remains
rejected.
...
Large-row Linear bias-gradient and LayerNorm affine-gradient reductions in the Tile CUDA library use chunked Tile atomic reductions instead of cuBLAS SGEMV on the fallback route. Linear bias gradients default to 256-row chunks after the CUDA 13.3 dedicated RTX 5090 same-script benchmark measured `0.995306x` train-loop wall time and `1.005073x` tokens/sec versus the older 512-row route. LayerNorm affine gradients default to 128-row chunks after the matching benchmark measured `0.997750x` train-loop wall time and `1.002270x` tokens/sec versus the older 256-row route. Set `NFN_TILE_CUDA_LAYERNORM_AFFINE_ROW_CHUNK_SIZE=N` (or `NFN_NATIVE_GPT_LAYERNORM_AFFINE_ROW_CHUNK_SIZE` / `NFN_NATIVE_GPT2_LAYERNORM_AFFINE_ROW_CHUNK_SIZE`) to compare other LayerNorm row chunk sizes in the paired benchmark harness; `NFN_SM120_NATIVE_CANDIDATE_PROFILE=layernorm_affine_row_chunk_128` now forces its baseline to 256 rows so the promoted default can still be rechecked against the older route. The 64-row and 96-row profiles are available as rejected diagnostics: 64 rows improved train-loop wall to `0.998045x` but failed hot-stage gates at `1.004276x` MLP projection and `1.000446x` LM-head backward; 96 rows improved train-loop wall to `0.999112x` but failed the same gates at `1.000296x` and `1.000002x`. The 128-row route remains the LayerNorm default. This keeps the default GPT `batch=64`, `seq=1024` backward pass on the faster local reduction route while providing enough row-chunk parallelism for the dedicated RTX 5090 compute path; small reductions can still use cuBLAS where selected.
