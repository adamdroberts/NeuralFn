...
For the composed MoE-JEPA native train-step slice, use `nfn_moe_jepa_evo_native_train --train-moe-jepa-loop-step --tile-ops-lib PATH` or `nfn-native-train --base-model moe-jepa-evo --native-cuda-train-moe-jepa-loop-step`; the bare `nfn_moe_jepa_evo_native_train ...` and `nfn-native-train --base-model moe-jepa-evo ...` paths also enter this slice by default. It runs the standard MoE full-loop train-step and AR+JEPA+router objective substeps through compiled native C++ and reports substep JSON without Torch or graph-editor tensor flow. The payload keeps `production_training_loop: false` and `production-family-forward-backward-optimizer-loop` in `native_training_missing_requirements` until the full dataset loop, checkpoint cadence, and inference metadata path are implemented.
...
the first step is still running. The trainer now writes the resolved schedule and
