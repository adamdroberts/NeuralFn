...
Native GPT attention now defaults to a packed-QKV SM120 TK bridge on the compiled Tile-CUDA path. The trainer writes the no-bias QKV GEMM output as packed BF16, applies QKV bias in-place, runs packed TK attention directly over the row-major packed QKV tensor, and feeds the packed BF16 attention output directly into the attention projection GEMM and dWeight accumulation. Backward uses the matching packed-QKV TK gradient bridge to return row-major `grad_qkv`. Set `NFN_NATIVE_GPT2_PACKED_QKV_ATTENTION=0` only when comparing against the older split-to-heads bridge. Plan and runtime JSON report `packed_qkv_attention_enabled`, packed BF16 scratch bytes, `packed_qkv_float_attention_tape_elided`, `packed_qkv_float_attention_tape_elements_elided`, packed QKV layout strategies, `attention_projection_input_strategy`, `attention_packed_output_unpack_strategy`, and `attention_backward_strategy: "tk-sm120-packed-qkv-bf16-backward-bridge"` when this default path is active. For memory-bounded profiling, set `NFN_NATIVE_GPT2_STORE_PACKED_ATTENTION_ACTIVATIONS=1` and optionally cap the number of earlier blocks with `NFN_NATIVE_GPT2_STORE_PACKED_ATTENTION_BLOCKS=N`; this stores packed BF16 QKV and packed O for those blocks, reuses them during backward, and reports `packed_attention_activation_storage_strategy`, `stored_packed_attention_*`, and `block_recompute_saved_packed_attention` timing fields. After packed float tape elision the full 11-block store fits the default `64 x 1024` RTX 5090 run, but it remains opt-in because the current saved-backward bridge is slower than recompute.
