...
switches for token-weight initialization experiments. On the CUDA 13.3.33
dedicated RTX 5090 retest, vector4-strided regressed token init to `1.010306x`
while threaded token init improved that substage to `0.930195x` but regressed
overall setup wall time to `1.039387x`; the default fast int32 Tile initializer
therefore remains the supported path.
