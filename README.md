...
Semantic-router MoE preflight now requires the raw Tile ABI for semantic hash routing, chunk-route broadcast, semantic-alignment item losses, attentionless decoding, expert bias add, standard top-k/expert-route broadcast, route-balance loss, latent MSE, and AdamW. `required-tile-symbols-present` means the loaded Tile ops library exports that semantic/MoE/JEPA kernel surface; `family-native-loop-missing` still means the model-specific semantic trainer loop has not landed yet.
