...
Native GPT attention now defaults to a packed-QKV SM120 TK bridge on the compiled Tile-CUDA path. The trainer writes the no-bias QKV GEMM output as packed BF16, applies QKV bias in-place, runs packed TK attention directly over the row-major packed QKV tensor, and feeds the packed BF16 attention output directly into the attention projection GEMM and dWeight accumulation. Backward uses the matching packed-QKV TK gradient bridge to return row-major `grad_qkv`. Set `NFN_NATIVE_GPT_PACKED_QKV_ATTENTION=0` only when comparing against the older split-to-heads bridge. Plan and runtime JSON report `packed_qkv_attention_enabled`, packed BF16 scratch bytes, `packed_qkv_float_attention_tape_elided`, `packed_qkv_float_attention_tape_elements_elided`, packed QKV layout strategies, `attention_projection_input_strategy`, `attention_packed_output_unpack_strategy`, and `attention_backward_strategy: "tk-sm120-packed-qkv-bf16-backward-bridge"` when this default path is active. The packed route now saves packed BF16 QKV and packed O for the first three earlier blocks by default, reuses those tensors during backward, and reports `packed_attention_activation_storage_strategy: "packed-qkv-o-bf16-forward-store-direct-backward"`, `stored_packed_attention_*`, and `block_recompute_saved_packed_attention` timing fields. Set `NFN_NATIVE_GPT_STORE_PACKED_ATTENTION_ACTIVATIONS=0` for the previous lower-memory recompute path, or set `NFN_NATIVE_GPT_STORE_PACKED_ATTENTION_BLOCKS=N` to tune the cap; the GPT-2-prefixed names remain fallbacks.
