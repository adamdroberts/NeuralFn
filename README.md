...
semantic-router MoE, DeepSeek-V4, `nfn train`, native inference, and SDK
native training handoff surfaces still run under an import blocker for Torch,
NumPy, `tiktoken`, dataset manager imports, and `nfn_impl`.
...
GPT-2 transformer-LM startup also initializes the tied token embedding/LM-head weight on device through `nfn_native_tile_init_gpt2_token_weight_fast_float32` instead of constructing a 154 MB host float matrix and copying it to the GPU. The default fast Tile initializer uses int32 Tile indices when the table fits in int32 plus a power-of-two deterministic value pattern over the full padded vocabulary table, avoiding the older modulo operation in the startup kernel; set `NFN_NATIVE_GPT_TOKEN_WEIGHT_FAST_INT32_INIT=0` only for paired comparison against the older int64-index Tile path, and set `NFN_NATIVE_GPT_TOKEN_WEIGHT_THREADED_INIT=1` only for paired comparison against the not-promoted threaded CUDA initializer. Training JSON reports `token_weight_init_strategy: "device-tile-power2-deterministic"` or `"device-tile-power2-deterministic-fused-bf16-shadow"`, `token_weight_threaded_init_enabled`, `token_weight_fast_int32_init_enabled`, and `token_weight_host_materialization: false`.
