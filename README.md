...
semantic MoE/JEPA, seq2seq, diffusion, Jamba, TTT, HNet byte-LM, and universal
...
For composed default train-step slices on the remaining non-dense GPT families, use `nfn_jamba_native_train --train-jamba-loop-step`, `nfn_ttt_llama_native_train --train-ttt-loop-step`, `nfn_hnet_lm_native_train --train-hnet-loop-step`, or `nfn_universal_llama_native_train --train-universal-loop-step` with `--tile-ops-lib PATH`. The unified aliases are `--native-cuda-train-jamba-loop-step`, `--native-cuda-train-ttt-loop-step`, `--native-cuda-train-hnet-loop-step`, and `--native-cuda-train-universal-loop-step`. Bare family commands enter the same slices by default and report substep JSON without Torch or graph-editor tensor flow.
