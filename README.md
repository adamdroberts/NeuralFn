...
`NFN_NATIVE_GPT_BF16_BLOCK_DWEIGHT_STAGING=1` is an opt-in profiling switch for the new `nfn_native_tile_linear_backward_weight_bias_accumulate_bf16_bits_bf16_bits_to_bf16_bits_float32` ABI. It accumulates QKV and MLP FC BF16/BF16 dWeights into BF16 staging buffers, then flushes them back to the normal float32 accumulation buffers before gradient clipping and AdamW. Leave it disabled for normal training; a paired dedicated-RTX-5090 benchmark measured the staging candidate about `1.0245x` slower than the current cuBLASLt bgrad default. Runtime JSON reports `block_dweight_bf16_staging_enabled`, staging allocation sizes, zero count, flush launches, and `block_dweight_bf16_staging_strategy`.
...
Native GPT startup now initializes the tied token FP32 master weight and its persistent BF16 LM-head shadow in one CUDA Tile ABI call, `nfn_native_tile_init_gpt2_token_weight_with_bf16_shadow_float32`, when `NFN_NATIVE_GPT_TOKEN_WEIGHT_BF16_SHADOW=1` (the default). Set `NFN_NATIVE_GPT_FUSE_TOKEN_WEIGHT_BF16_INIT=0` to reproduce the older two-pass startup path for paired benchmarks. Runtime JSON reports `token_weight_bf16_initial_refresh_fusion_enabled` and `token_weight_bf16_initial_refresh_elided`; `--startup-only` is the cleanest way to measure this setup-only change.
...
GPT-2 transformer-LM startup also initializes the tied token embedding/LM-head weight on device through `nfn_native_tile_init_gpt2_token_weight_float32` instead of constructing a 154 MB host float matrix and copying it to the GPU. That Tile initializer now uses 2048-element tiles for the full padded vocabulary table, reducing launch fanout during startup while preserving the deterministic initialization pattern. Training JSON reports `token_weight_init_strategy: "device-tile-deterministic"` and `token_weight_host_materialization: false`.
