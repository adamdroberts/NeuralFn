...
`nvidia-smi` before training. Set one of those variables to `dedicated`,
`auto`, or `dedicated-auto` when a benchmark should probe for a
display-disabled NVIDIA GPU, or set `CUDA_VISIBLE_DEVICES` explicitly to pin a
different device.
