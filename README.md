...
For composed default train-step slices on the remaining non-dense GPT families, use `nfn_jamba_native_train --train-jamba-loop-step` or `nfn_hnet_lm_native_train --train-hnet-loop-step` with `--tile-ops-lib PATH`. The unified aliases are `--native-cuda-train-jamba-loop-step` and `--native-cuda-train-hnet-loop-step`. Bare Jamba and HNet family commands enter the same slices by default and report substep JSON without Torch or graph-editor tensor flow.
