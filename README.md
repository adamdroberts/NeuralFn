...
block slice, packed-QKV RoPE attention-block integration, and family
parameter-layout/checkpoint/inference metadata, while their missing list keeps
the full LLaMA block forward/backward loop.
...
residual add in one compiled CUDA Tile preflight. These are coverage gates for
the LLaMA loop wiring, not a complete LLaMA trainer.
...
For the LLaMA packed-QKV attention slice, use `nfn_llama_native_train --smoke-llama-packed-attention-step --tile-ops-lib PATH` or `nfn-native-train --base-model llama --native-cuda-smoke-llama-packed-attention-step`; it launches packed-QKV BF16 causal attention with saved LSE and backward-to-QKV without Torch or graph-editor tensor flow. For the forward attention-block composition slice, use `nfn_llama_native_train --smoke-llama-attention-block-step --tile-ops-lib PATH` or `nfn-native-train --base-model llama --native-cuda-smoke-llama-attention-block-step`. For the RoPE attention-block integration slice, use `nfn_llama_native_train --smoke-llama-rope-attention-block-step --tile-ops-lib PATH` or `nfn-native-train --base-model llama --native-cuda-smoke-llama-rope-attention-block-step`; it launches RMSNorm, QKV projection, head-major QKV split, RoPE, SDPA, and residual add through raw CUDA Tile ABI calls.
