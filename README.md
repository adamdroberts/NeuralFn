...
The legacy graph-backed family inference helpers for LLaMA-fast, LLaMA-megakernel, MixLLaMA-fast, and NanoGPT now follow the same parser/help startup discipline as GPT inference. Running `python cli/scripts/infer_llama_fast.py --help`, `python cli/scripts/infer_llama_megakernel.py --help`, `python cli/scripts/infer_mixllama_fast.py --help`, or `python cli/scripts/infer_nanogpt.py --help` stays in the lightweight CLI layer and does not import Torch, NumPy, tokenizers, the dataset manager, or graph-backed runtime modules.
