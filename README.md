...
For the remaining composed default train-step slice on non-dense GPT families, use `nfn_hnet_lm_native_train --train-hnet-loop-step` with `--tile-ops-lib PATH`. The unified alias is `--native-cuda-train-hnet-loop-step`. Bare HNet family commands enter the same slice by default and report substep JSON without Torch or graph-editor tensor flow.
