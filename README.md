...
`eps=1e-08`, and `weight_decay=0.02`. LLaMA-family and dense-JEPA runs now
default to native token-shard dataset loops with sampled AR CE plus their
...
For composed default train-step slices on the remaining non-dense GPT families, use `nfn_jamba_native_train --train-jamba-loop-step`, `nfn_seq2seq_native_train --train-seq2seq-loop-step`, `nfn_diffusion_native_train --train-diffusion-loop-step`, `nfn_ttt_llama_native_train --train-ttt-loop-step`, `nfn_hnet_lm_native_train --train-hnet-loop-step`, or `nfn_universal_llama_native_train --train-universal-loop-step` with `--tile-ops-lib PATH`. The unified aliases are `--native-cuda-train-jamba-loop-step`, `--native-cuda-train-seq2seq-loop-step`, `--native-cuda-train-diffusion-loop-step`, `--native-cuda-train-ttt-loop-step`, `--native-cuda-train-hnet-loop-step`, and `--native-cuda-train-universal-loop-step`. Bare family commands enter the same slices by default and report substep JSON without Torch or graph-editor tensor flow.
