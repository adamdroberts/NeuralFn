...
defaulting to `dedicated`, which selects a display-disabled NVIDIA GPU through
`nvidia-smi` and falls back to ordinal `0` if that probe is unavailable. Set
one of those variables, or `CUDA_VISIBLE_DEVICES` itself, to an explicit ordinal
when a run should skip the selector.
