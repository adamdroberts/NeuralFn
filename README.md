...
`tools/check_native_no_torch_deps.py` verifies both wrapper-style dry-run/preflight commands and no-flag direct execution for these guarded legacy scripts under an import blocker, so a normal script invocation cannot regress into Torch, NumPy, tokenizer, dataset-manager, or `nfn_impl` startup before crossing the native C++ boundary.
