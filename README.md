...
`tools/check_native_no_torch_deps.py` verifies both wrapper-style dry-run/preflight commands and no-flag direct execution for these guarded legacy scripts under an import blocker, and also imports the native-only GPT-2-evo, NanoGPT, MixLLaMA, and DeepSeek-V4 shim modules under the same blocker. A normal script invocation or shim import therefore cannot regress into Torch, NumPy, tokenizer, dataset-manager, `train_gpt_native`, or `nfn_impl` startup before crossing the native C++ boundary.
