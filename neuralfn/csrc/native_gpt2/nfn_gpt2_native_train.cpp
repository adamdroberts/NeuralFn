...
int run_transformer_lm_training_json(
    ...
    using LinearBackwardInputDgeluWeightBf16BitsOnlyFn = int (*)(
        const float*, const std::uint16_t*, const std::uint16_t*, std::uint16_t*, float*,
        std::int64_t, std::int64_t, std::int64_t, void*);
    using LinearBackwardInputDgeluBf16BitsWeightBf16BitsOnlyFn = int (*)(
    ...
    LinearBackwardInputDgeluBf16BitsWeightBf16BitsOnlyFn
            ...
            if (!missing_symbols.empty()) {
                ...
                linear_backward_input_dgelu_weight_bf16_bits_only =
                    load_symbol<LinearBackwardInputDgeluWeightBf16BitsOnlyFn>(
                        tile_handle,
                        "nfn_native_tile_linear_backward_input_dgelu_weight_bf16_bits_only_float32");
                linear_backward_input_dgelu_bf16_bits_weight_bf16_bits_only =
                    load_symbol<LinearBackwardInputDgeluBf16BitsWeightBf16BitsOnlyFn>(
                        tile_handle,
                        "nfn_native_tile_linear_backward_input_dgelu_bf16_bits_weight_bf16_bits_only_float32");
    ...
    struct NamedFloatArenaRequest {
        float** ptr = nullptr;
        std::int64_t elements = 0;
        const char* name = "";
    };
    for (const NamedFloatArenaRequest& item : {
             NamedFloatArenaRequest{&token_weight, kTokenWeightElements, "token_weight"},
             {&position_weight, position_weight_elements, "position_weight"},
             {&residual_scale, 1, "residual_scale"},
             {&lnf_weight, kDim, "lnf.weight"},
             {&lnf_bias, kDim, "lnf.bias"},
             {&accum_grad_token_weight, kTokenWeightElements, "token_weight.accum_grad"},
             {&accum_grad_position_weight, position_weight_elements, "position_weight.accum_grad"},
             {&accum_grad_lnf_weight, kDim, "lnf.weight.accum_grad"},
             {&accum_grad_lnf_bias, kDim, "lnf.bias.accum_grad"},
             {&token_avg, kTokenWeightElements, "token_weight.avg"},
             {&token_avg_sq, kTokenWeightElements, "token_weight.avg_sq"},
             {&position_avg, position_weight_elements, "position_weight.avg"},
             {&position_avg_sq, position_weight_elements, "position_weight.avg_sq"},
             {&lnf_weight_avg, kDim, "lnf.weight.avg"},
             ...
             {&lnf_out, activation_elements, "lnf.out"},
             {&logits, logit_workspace_elements, "lm_head.float_logits"},
             {&loss_partials, loss_partial_count, "loss_partials"},
             {&loss_reduce_a, loss_partial_count, "loss_reduce_a"},
             {&loss_reduce_b, loss_partial_count, "loss_reduce_b"},
             {&loss_total, 1, "loss_total"},
             {&row_max, lm_head_chunk_rows, "lm_head.row_max"},
             ...
             {&grad_residual1_from_mlp,
              ...
              "residual1.grad_from_mlp"},
             {&grad_residual1, activation_elements, "residual1.grad"},
             {&grad_attn_out,
              bf16_attention_grad_out_handoff_enabled ? 0 : activation_elements,
              "attention.grad_out"},
             {&grad_qkv, grad_qkv_float_scratch_elements, "attention.qkv.float_grad_scratch"},
             {&grad_ln1, activation_elements, "ln1.grad_input"},
             {&grad_x_from_attn,
              fuse_ln_backward_residual_enabled ? 0 : activation_elements,
              "embedding_residual.grad_from_attention"},
             {&grad_x, activation_elements, "embedding_residual.grad"},
