...
struct Config {
    ...
    int lm_head_row_chunk_size = 16384;
...
void print_usage(const char* program) {
    std::cout
        ...
        << "  --lm-head-row-chunk-size N        Tied LM-head full-vocab row chunk size for the Tile-CUDA transformer loop; default 16384.\n"
...
std::vector<StagePlan> build_gpt2_stage_plan(const Config& cfg) {
    ...
    for (int layer = cfg.num_layers - 1; layer >= 0; --layer) {
        ...
        add(prefix + "attn.sdpa.backward_to_qkv_from_merged_grad", "backward", "nfn_native_tile_scaled_dot_product_attention_backward_to_qkv_from_merged_grad_float32", qkv);
...
bool print_tile_plan(
    ...
    std::cout
        ...
        << "  \"qkv_forward_layout_kernel_launches_per_block\": 1,\n"
        << "  \"qkv_forward_layout_legacy_launches_per_block\": 4,\n"
        << "  \"qkv_forward_layout_launches_elided_per_block\": 3,\n"
        << "  \"qkv_bias_layout_strategy\": \"fused-qkv-bias-split-to-heads\",\n"
        ...
        << "  \"qkv_backward_layout_strategy\": \"fused-heads-to-qkv\",\n"
        ...
        << "  \"attention_backward_qkv_bridge_strategy\": \"fused-bf16-heads-to-row-qkv\",\n"
        << "  \"attention_backward_qkv_bridge_kernel_launches_per_block\": 1,\n"
        ...
        << "  \"attention_backward_strategy\": \"tk-sm120-bf16-reuse-forward-workspace-bridge\",\n"
        << "  \"attention_backward_reuses_forward_workspace\": true,\n"
        << "  \"attention_backward_uses_saved_forward_workspace\": false,\n"
        << "  \"attention_activation_storage_strategy\": \"disabled-by-default-opt-in\",\n"
...
int run_transformer_lm_training_json(
    ...
    const std::int64_t stored_attention_block_count =
        store_attention_activations_enabled && trained_layers > 0 ? trained_layers - 1 : 0;
    ...
    auto build_adamw_descriptors = [&]() {
        ...
        auto add = [&](float* param, float* grad, float* avg, float* avg_sq, std::int64_t elements, float decay) {
            if (!error.empty()) {
                return;
            }
        ...
        for (TransformerBlockParams& block : blocks) {
            ...
            add(block.qkv_weight, block.accum_grad_qkv_weight, block.qkv_avg, block.qkv_avg_sq, kQkvWeightElements, kWeightDecay);
            add(block.qkv_bias, block.accum_grad_qkv_bias, block.qkv_bias_avg, block.qkv_bias_avg_sq, kQkvDim, 0.0f);
            add(block.attn_proj_weight, block.accum_grad_attn_proj_weight, block.attn_proj_avg, block.attn_proj_avg_sq, kAttnProjWeightElements, kWeightDecay);
            ...
            add(block.fc_weight, block.accum_grad_fc_weight, block.fc_avg, block.fc_avg_sq, kFcWeightElements, kWeightDecay);
            add(block.fc_bias, block.accum_grad_fc_bias, block.fc_bias_avg, block.fc_bias_avg_sq, kHidden, 0.0f);
            add(block.mlp_proj_weight, block.accum_grad_mlp_proj_weight, block.mlp_proj_avg, block.mlp_proj_avg_sq, kMlpProjWeightElements, kWeightDecay);
    ...
    auto backward_block = [&](TransformerBlockParams& block,
        ...
        run_timed_stage("block_backward.mlp_proj", [&]() {
            run_timed_stage("block_backward.mlp_proj.dweight", [&]() {
                if (stored_mlp != nullptr) {
                    if (error.empty()) run(linear_backward_weight_accumulate_bf16_bits(stored_mlp->act, incoming_grad, block.accum_grad_mlp_proj_weight, rows, kHidden, kDim, nullptr), label + ".mlp.proj.backward_weight.accumulate.bf16_bits");
                } else {
            ...
            run_timed_stage("block_backward.mlp_proj.bias", [&]() {
        ...
        run_timed_stage("block_backward.mlp_fc", [&]() {
            run_timed_stage("block_backward.mlp_fc.dweight", [&]() {
                if (stored_mlp != nullptr) {
                    if (error.empty()) run(linear_backward_weight_accumulate_bf16_bits(stored_mlp->ln2_out, grad_fc_out, block.accum_grad_fc_weight, rows, kDim, kHidden, nullptr), label + ".mlp.fc.backward_weight.accumulate.bf16_bits");
                } else {
            ...
            run_timed_stage("block_backward.mlp_fc.bias", [&]() {
        ...
        run_timed_stage("block_backward.qkv", [&]() {
            run_timed_stage("block_backward.qkv.dweight", [&]() {
            ...
            run_timed_stage("block_backward.qkv.bias", [&]() {
    ...
    auto run_backward_microbatch = [&](bool record_train_loss, float accumulation_scale) -> double {
        zero_gradients();
    ...
    auto forward_backward_update = [&](std::int64_t step, bool record_train_loss) -> double {
        ...
        for (std::int64_t accum_index = 0; accum_index < grad_accum_steps && error.empty(); ++accum_index) {
            ...
            const double microbatch_loss_sum = run_backward_microbatch(record_train_loss, accumulation_scale);
    ...
    std::cout
        ...
        << (stored_attention_backward_kernel_launches > 0 ? "true" : "false") << ",\n"
        ...
        << (stored_attention_block_count > 0 ? "tk-bf16-forward-store-saved-backward" : "disabled")
        << "\",\n"
        ...
        << (stored_attention_block_count > 0 ? "true" : "false") << ",\n"
