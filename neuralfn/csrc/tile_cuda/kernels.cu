...
bool token_weight_padded_bf16_pattern_enabled() {
  static const bool value = []() {
    const char* raw = std::getenv("NFN_TILE_CUDA_TOKEN_WEIGHT_PADDED_BF16_PATTERN");
    if (raw == nullptr) {
      raw = std::getenv("NFN_NATIVE_GPT_TOKEN_WEIGHT_PADDED_BF16_PATTERN");
    }
    if (raw == nullptr) {
...
bool lm_head_ce_no_loss_vec8_normal_store_specialized_enabled() {
  static const bool value = []() {
    const char* raw = std::getenv("NFN_TILE_CUDA_LM_HEAD_CE_NO_LOSS_VEC8_NORMAL_STORE_SPECIALIZED");
    if (raw == nullptr) {
      raw = std::getenv("NFN_NATIVE_GPT_LM_HEAD_CE_NO_LOSS_VEC8_NORMAL_STORE_SPECIALIZED");
    }
    if (raw == nullptr) {
      raw = std::getenv("NFN_NATIVE_GPT2_LM_HEAD_CE_NO_LOSS_VEC8_NORMAL_STORE_SPECIALIZED");
    }
    if (raw == nullptr) {
      return true;
    }
...
__device__ __forceinline__ float nvfp4_packed_value_device(
    const std::uint8_t* __restrict__ packed,
    const std::uint8_t* __restrict__ block_scales_e4m3,
    float tensor_scale,
    std::int64_t index) {
  constexpr std::int64_t kNvfp4BlockSize = 16;
  const std::uint8_t packed_byte = packed[index / 2];
  const std::uint8_t code =
      (index & 1) == 0 ? (packed_byte & 0x0f) : ((packed_byte >> 4) & 0x0f);
  const std::int64_t scale_block = index / kNvfp4BlockSize;
  const float safe_tensor_scale = tensor_scale > 0.0f ? tensor_scale : 1.0f;
  const float block_scale =
      nvfp4_e4m3fn_to_float_device(block_scales_e4m3[scale_block]) * safe_tensor_scale;
  return nvfp4_e2m1_code_to_float_device(code) * block_scale;
}
...
bool trainer_linear_tk_dgelu_dinput_shape_disabled(
    int m,
    int n,
    int k,
    cublasOperation_t op_a,
    cublasOperation_t op_b) {
  static const LinearShapeStat disabled_shape = []() {
    const char* value = std::getenv("NFN_TILE_CUDA_LINEAR_TK_DGELU_DINPUT_DISABLE_SHAPE");
    if (value == nullptr) {
      value = std::getenv("NFN_NATIVE_LINEAR_TK_DGELU_DINPUT_DISABLE_SHAPE");
    }
    LinearShapeStat shape{};
    parse_linear_shape_token(value, &shape);
    return shape;
...
bool tk_linear_backward_input_dgelu_bf16_bits_float32(
  ...
  constexpr cublasOperation_t kOpB = CUBLAS_OP_N;
  if (!trainer_linear_tk_gemm_enabled() ||
      trainer_linear_tk_dgelu_dinput_shape_disabled(input_dim, rows, output_dim, kOpA, kOpB)) {
    return false;
  }
...
__global__ void init_gpt2_token_weight_vector4_with_bf16_shadow_padded_float32_kernel(
  ...
  if (idx + 3 < total_n) {
    ...
    if (idx + 3 < public_n) {
      ...
      const float4 pattern = gpt2_token_weight_init_float_pattern4(bucket);
      reinterpret_cast<float4*>(values)[idx / 4] = pattern;
      reinterpret_cast<ushort4*>(shadow_bf16_bits)[idx / 4] = precomputed_bf16_pattern
          ? gpt2_token_weight_init_bf16_pattern4(bucket)
          : make_ushort4(
                bf16_bits_from_float(pattern.x),
                bf16_bits_from_float(pattern.y),
  ...
  for (std::int64_t vector_idx =
    ...
    if (idx + 3 < total_n) {
      ...
      if (idx + 3 < public_n) {
        ...
        const float4 pattern = gpt2_token_weight_init_float_pattern4(bucket);
        reinterpret_cast<float4*>(values)[vector_idx] = pattern;
        reinterpret_cast<ushort4*>(shadow_bf16_bits)[vector_idx] = precomputed_bf16_pattern
            ? gpt2_token_weight_init_bf16_pattern4(bucket)
            : make_ushort4(
                  bf16_bits_from_float(pattern.x),
                  bf16_bits_from_float(pattern.y),
...
__tile_global__ void moe_swiglu_backward_float32_kernel(
    const float* __restrict__ x,
    const float* __restrict__ route_weights,
    const std::int64_t* __restrict__ route_indices,
    const float* __restrict__ w1,
  ...
  const int bx = ct::bid().x;
  using IndexTile = ct::tile<std::int64_t, decltype(ct::shape{1024_ic})>;
  auto token = ct::iota<IndexTile>() + ct::full<IndexTile>(static_cast<std::int64_t>(bx) * kTileSize);
  auto valid_token = token < ct::full<IndexTile>(tokens);
  auto dim_tile = ct::full<IndexTile>(dim);
  auto hidden_tile = ct::full<IndexTile>(hidden_dim);
  auto top_k_tile = ct::full<IndexTile>(top_k);
  auto experts_tile = ct::full<IndexTile>(experts);
  ...
  for (std::int64_t d_in = 0; d_in < dim; ++d_in) {
    ...
    for (std::int64_t k = 0; k < top_k; ++k) {
      ...
      for (std::int64_t h = 0; h < hidden_dim; ++h) {
        ...
        auto one = ct::full<decltype(gate)>(1.0f);
        auto sigmoid = one / (one + ct::exp(-gate));
        auto silu = gate * sigmoid;
        auto dsilu = sigmoid * (one + gate * (one - sigmoid));
        auto hidden = silu * up;
        auto grad_hidden = ct::full<decltype(route_weight)>(0.0f);
        for (std::int64_t d_out = 0; d_out < dim; ++d_out) {
          auto d_out_tile = ct::full<IndexTile>(d_out);
          auto grad_expert_out = route_weight * ct::load_masked(grad_out + token * dim_tile + d_out_tile, valid);
          auto w2_offset = (expert * hidden_tile + h_tile) * dim_tile + d_out_tile;
          if (d_in == 0) {
            ct::atomic_add_masked<ct::memory_order::relaxed>(
                grad_w2 + w2_offset,
                hidden * grad_expert_out,
                valid);
          }
          grad_hidden = grad_hidden + grad_expert_out * ct::load_masked(w2 + w2_offset, valid);
        }
        auto grad_up = grad_hidden * silu;
        auto grad_gate = grad_hidden * up * dsilu;
        auto d_in_tile = ct::full<IndexTile>(d_in);
        auto x_in = ct::load_masked(x + token * dim_tile + d_in_tile, valid);
        auto w13_offset = (expert * dim_tile + d_in_tile) * hidden_tile + h_tile;
        ct::atomic_add_masked<ct::memory_order::relaxed>(
            grad_w1 + w13_offset,
            x_in * grad_gate,
            valid);
        ...
        grad_x_acc =
            ...
            ct::select(
                valid,
                grad_gate * ct::load_masked(w1 + w13_offset, valid) +
                    grad_up * ct::load_masked(w3 + w13_offset, valid),
                ct::full<decltype(grad_x_acc)>(0.0f));
      }
    }
...
__global__ void linear_nvfp4_input_weight_bf16_bits_float32_kernel(
...
__global__ void linear_backward_weight_accumulate_nvfp4_input_float32_kernel(
  ...
  for (std::int64_t row_base = 0; row_base < rows; row_base += kTile) {
    ...
    const int grad_out_col = static_cast<int>(blockIdx.y) * kTile + threadIdx.x;
    if (row < rows && grad_out_col < output_dim) {
      grad_tile[threadIdx.y][threadIdx.x] = grad_out[row * output_dim + grad_out_col];
    } else {
      grad_tile[threadIdx.y][threadIdx.x] = 0.0f;
    }
...
__global__ void token_cross_entropy_backward_inplace_strided_no_pad_zero_bf16_bits_u16_targets_vec8_normal_store_kernel(
    std::uint16_t* __restrict__ logits,
    const std::uint16_t* __restrict__ targets,
    std::int64_t rows,
    std::int64_t vocab,
...
#if defined(NFN_TILE_CUDA_LM_HEAD_TRUE_FUSED_WMMA) && \
    NFN_TILE_CUDA_LM_HEAD_TRUE_FUSED_MAT_TILE == 16
  constexpr int kCeWarpSize = 32;
  constexpr int kCeWarpsPerBlock = kLmHeadTrueFusedRequiredThreads / kCeWarpSize;
  const int ce_warp = static_cast<int>(threadIdx.x) / kCeWarpSize;
  const int ce_lane = static_cast<int>(threadIdx.x) - ce_warp * kCeWarpSize;
  if (ce_warp < kCeWarpsPerBlock) {
    for (std::int64_t row =
             static_cast<std::int64_t>(blockIdx.x) * kCeWarpsPerBlock + ce_warp;
         row < rows;
         row += static_cast<std::int64_t>(gridDim.x) * kCeWarpsPerBlock) {
      std::uint16_t* row_logits = logits + row * row_stride;
      constexpr std::int64_t kVec = 8;
      const std::int64_t aligned_vocab = vocab & ~(kVec - 1);
      float thread_max = -INFINITY;
      for (std::int64_t col = static_cast<std::int64_t>(ce_lane) * kVec;
           col < aligned_vocab;
           col += static_cast<std::int64_t>(kCeWarpSize) * kVec) {
        const int4 packed = load_bf16_vec8(row_logits + col);
#pragma unroll
        for (int offset = 0; offset < 8; ++offset) {
          thread_max = fmaxf(thread_max, bf16_bits_to_f32_device(int4_u16_at(packed, offset)));
      ...
      for (std::int64_t col = aligned_vocab + ce_lane; col < vocab; col += kCeWarpSize) {
        thread_max = fmaxf(thread_max, bf16_bits_to_f32_device(row_logits[col]));
      }
      const float row_max = warp_reduce_max_f32(thread_max);
...
#pragma unroll
        for (int offset = 0; offset < 8; ++offset) {
          const float value = bf16_bits_to_f32_device(int4_u16_at(packed, offset));
          thread_sum += cross_entropy_exp_device(value - row_max, kLmHeadTrueFusedCeExp2);
        }
      }
      for (std::int64_t col = aligned_vocab + ce_lane; col < vocab; col += kCeWarpSize) {
        thread_sum += cross_entropy_exp_device(
            bf16_bits_to_f32_device(row_logits[col]) - row_max,
            kLmHeadTrueFusedCeExp2);
      }
      const float row_denom = warp_reduce_sum_f32(thread_sum);
      const std::uint16_t target = targets[row];

      if (!no_loss && ce_lane == 0 && row_losses != nullptr) {
...
#if defined(NFN_TILE_CUDA_LM_HEAD_TRUE_FUSED_WMMA) && \
    NFN_TILE_CUDA_LM_HEAD_TRUE_FUSED_MAT_TILE == 16
  constexpr int kWmmaWarpSize = 32;
  constexpr int kWmmaWarpsPerBlock = kLmHeadTrueFusedRequiredThreads / kWmmaWarpSize;
  __shared__ float wmma_tile[kWmmaWarpsPerBlock][16][16];
  const int warp_id = lane / kWmmaWarpSize;
  const int warp_lane = lane - warp_id * kWmmaWarpSize;
  const bool wmma_warp = warp_id < kWmmaWarpsPerBlock;
#endif
...
#if defined(NFN_TILE_CUDA_LM_HEAD_TRUE_FUSED_WMMA) && \
    NFN_TILE_CUDA_LM_HEAD_TRUE_FUSED_MAT_TILE == 16
  for (std::int64_t tile_index =
           static_cast<std::int64_t>(blockIdx.x) * kWmmaWarpsPerBlock + warp_id;
       tile_index < hidden_tiles;
       tile_index += static_cast<std::int64_t>(gridDim.x) * kWmmaWarpsPerBlock) {
    if (!wmma_warp) {
      continue;
    }
    ...
    if (full_tile) {
      ...
      wmma::store_matrix_sync(&wmma_tile[warp_id][0][0], c_frag, 16, wmma::mem_row_major);
      __syncwarp();
      for (int elem = warp_lane; elem < kMatTile * kMatTile; elem += kWmmaWarpSize) {
        ...
        float sum = wmma_tile[warp_id][out_row][out_col];
    ...
    for (int elem = warp_lane; elem < kMatTile * kMatTile; elem += kWmmaWarpSize) {
      const int out_row = elem / kMatTile;
      const int out_col = elem - out_row * kMatTile;
      const std::int64_t row = row_tile * kMatTile + out_row;
      const std::int64_t dim = dim_tile * kMatTile + out_col;
      if (row >= rows || dim >= hidden_dim) {
        continue;
      }
      float sum = 0.0f;
      for (std::int64_t col = 0; col < vocab; ++col) {
        sum += bf16_bits_to_f32_device(logits[row * row_stride + col]) *
            bf16_bits_to_f32_device(token_weight_bf16[col * hidden_dim + dim]);
      }
...
#pragma unroll
  ...
  for (std::int64_t tile_index =
    ...
    for (int elem = warp_lane; elem < kMatTile * kMatTile; elem += kWmmaWarpSize) {
      ...
      const std::int64_t store_col = vocab_tile * kMatTile + out_row;
      const std::int64_t store_dim = dim_tile * kMatTile + out_col;
      if (store_col >= vocab || store_dim >= hidden_dim) {
        continue;
      }
      float sum = 0.0f;
...
void launch_moe_swiglu_backward_float32(
  ...
  moe_swiglu_backward_float32_kernel<<<blocks, 1, 0, stream>>>(
      x,
      route_weights,
      route_indices,
      w1,
...
void launch_linear_nvfp4_input_weight_bf16_float32(
  ...
  constexpr int threads = 256;
  const int blocks = static_cast<int>((n + threads - 1) / threads);
  linear_nvfp4_input_weight_bf16_bits_float32_kernel<<<blocks, threads, 0, stream>>>(
      x_nvfp4_packed,
      x_block_scales_e4m3,
      x_tensor_scale,
      weight_bf16_bits,
...
void launch_linear_backward_weight_accumulate_nvfp4_input_float32_beta(
  ...
  const dim3 grid(
      ...
      static_cast<unsigned int>((output_dim + kTile - 1) / kTile),
      1);
  linear_backward_weight_accumulate_nvfp4_input_float32_kernel<<<grid, block, 0, stream>>>(
      x_nvfp4_packed,
      x_block_scales_e4m3,
      x_tensor_scale,
      grad_out,
...
void launch_token_cross_entropy_backward_inplace_strided_no_pad_zero_bf16_bits_u16_targets_with_workspace(
  ...
  if (lm_head_ce_no_loss_vec8_normal_store_specialized_enabled() &&
      threads == 1024 &&
      vec_loads &&
      !vec_stores &&
      !scalar_streaming_stores &&
      !use_exp2) {
    token_cross_entropy_backward_inplace_strided_no_pad_zero_bf16_bits_u16_targets_vec8_normal_store_kernel<<<static_cast<int>(rows), threads, 0, stream>>>(
        logits, targets, rows, vocab, row_stride, loss_scale, lm_head_ce_reverse_rows_enabled());
    return;
  }
