...
- [ ] Extend the native GPT checkpoint sampler from one-token CUDA Tile inference to an autoregressive multi-token loop and text-prompt tokenization path so native `.bin` checkpoint inference is fully Torch-free.
