...
- [ ] Close the remaining SM120 parity gap with new kernel work, not more default-switch promotion. The next high-value implementation target is the LM-head classifier/backward contract reported in runtime JSON: fuse or otherwise co-schedule row-chunked BF16 logits, public-vocab CE/dlogits, dHidden, and dWeight so the default path is closer to llm.kittens' full-resident fused classifier without triggering the full-logit memory cliff.
  ...
  - 2026-06-22 promoted the no-loss LM-head CE specialization to the default
    ...
    3-step, 3-sample same-script RTX 5090 gate. The route change was visible in
    `lm_head_ce_no_loss_default_specialized_*` and `lm_head_ce_kernel_strategy`,
    and the candidate passed whole-loop gates at `0.986338x` train-loop wall,
    `0.977844x` steady-state CUDA-event wall, `1.013917x` tokens/sec, and
    `0.987834x` total wall versus the generic no-loss CE+dlogits baseline.
  ...
  - 2026-06-23 applied the same wrapper rejection enforcement to stale attention bisections: `bf16_attention_grad_out`, `bf16_attention_dprep_grad_out`, and `attention_dprep_float_hd64_specialized` still expose their route flags in dry-run plans, but real paired launches now require `NFN_SM120_NATIVE_ALLOW_REJECTED_CANDIDATE_PROFILE=1`.
  ...
  - 2026-06-24 refreshed `NFN_SM120_NATIVE_CANDIDATE_PROFILE=lm_head_loss_bins` after the CUDA reinstall on the dedicated RTX 5090. The same-script 3-step, 2-sample stage-timed rerun still proved the route (`lm_head_classifier_loss_bin_launch_count: 0 -> 48`) and improved `train_loop_cuda_event_steady_state_wall_ms_per_step` to `0.978956x`, `stage.lm_head_backward.total_ms` to `0.909128x`, and `stage.lm_head_backward.ce.total_ms` to `0.541309x`, but total train-loop wall stayed just outside the strict promotion gate at `1.000274x`. Keep it rejected by default and use it as evidence that the next useful LM-head work is the true cooperative logits/CE/dHidden/dWeight kernel, not another default toggle.
