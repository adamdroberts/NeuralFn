...
- [ ] Close the remaining measured SM120 throughput gap between the NeuralFn-owned `libnfn_native_train_tile_ops.so` loop and the `llm.kittens` SM120 reference script.
  ...
  - 2026-06-18 rejected promoting the existing fast int32-index Tile token-weight initializer as the default for the power-of-two GPT token-weight pattern. The first dedicated RTX 5090 startup-only native-vs-native 5-sample check with `NFN_NATIVE_GPT_TOKEN_WEIGHT_FAST_INT32_INIT=1` looked promising at `0.967818x` token-weight init and `0.992504x` total startup, but the rebuilt-library confirmation against an explicit opt-out measured `1.008932x` token-weight init, `1.010658x` setup wall, and `1.010691x` total startup. Keep the int32 path opt-in for bisection only.
