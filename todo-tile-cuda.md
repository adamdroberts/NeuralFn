...
- [x] Add the fused padded-vocab token-weight BF16-shadow init ABI without making it the default. `nfn_native_tile_init_gpt2_token_weight_fast_with_bf16_shadow_padded_float32` can initialize the public token rows and zero the padded-vocab tail plus BF16 shadow in one launch, and the native trainer exposes it behind `NFN_NATIVE_GPT_FUSE_TOKEN_WEIGHT_PADDED_INIT=1`. The padded kernel now writes public-vocab BF16 shadow rows with the precomputed deterministic pattern path instead of per-value BF16 conversion, but the 2026-06-25 dedicated RTX 5090 CUDA 13.3.33 startup-only gate still rejected it as a default because setup wall regressed to `1.010956x` and `setup.token_weight_init.total_ms` to `1.009406x` versus the current vector4 BF16-shadow route.
...
- [x] Revisit the CUDA 13.3 dedicated RTX 5090 parity baseline before retrying older candidates. The 2026-06-19 5-step same-script run measured llm.kittens at `2429.056 ms/step` and NeuralFn at `2536.100 ms/step`, or `1.044068x` train-loop wall time and `0.957209x` tokens/sec. The hottest remaining buckets were block backward (`6365.680 ms` over 5 steps) and LM-head backward (`3127.700 ms`), so parity work still needs new fused/cooperative kernel work rather than parameter-default retunes.
  ...
  - 2026-06-24 added the diagnostic cuBLASLt plan-prewarm mode selector (`NFN_NATIVE_GPT_PREWARM_CUBLASLT_PLAN_MODE=all|block_only|lm_head_only`) and rejected both selective modes on the dedicated RTX 5090. Startup-only probes confirmed the counters (`all`: attempted 9/skipped 0, `block_only`: attempted 8/skipped 1, `lm_head_only`: attempted 1/skipped 8). The 3-step, 2-sample stage-timed gate rejected `block_only` because steady-state CUDA-event time regressed to `1.002003x`, LM-head backward to `1.000030x`, MLP projection backward to `1.003987x`, and setup to `1.185925x`. `lm_head_only` saved setup to `0.947250x` but regressed train-loop wall to `1.011688x`, steady-state CUDA-event time to `1.001999x`, LM-head backward to `1.000280x`, block backward to `1.022887x`, and MLP projection backward to `1.021800x`. Keep the default mode at `all`; the parity gap still needs fused/cooperative kernels rather than selective plan prewarm.
...
- [x] Corrected token-weight startup bisection profiles after the CUDA 13.3 reinstall. The `token_weight_vector4_strided` wrapper profile now compares baseline `NFN_NATIVE_GPT_TOKEN_WEIGHT_VECTOR4_STRIDED_INIT=0` against candidate `=1`, and native JSON reports `token_weight_vector4_strided_init_requested` plus a distinct `device-vector4-strided-power2-deterministic[-fused-bf16-shadow]` strategy so the route-change gate can see the hidden Tile dispatch. The dedicated RTX 5090 2-sample startup gate passed at `0.949594x` setup wall and `0.986349x` token init, but remains diagnostic-only pending broader evidence.
- [x] Added first-class same-script coverage for the rejected BF16-pattern token-weight shadow writer. The `token_weight_bf16_pattern` wrapper profile now compares baseline `NFN_NATIVE_GPT_TOKEN_WEIGHT_BF16_PATTERN_INIT=0` against candidate `=1`, native JSON reports `token_weight_bf16_pattern_init_requested`, and the candidate strategy is labeled `device-vector4-power2-deterministic-fused-bf16-pattern-shadow` so route-change gates can distinguish it from the default conversion-based vector4 BF16-shadow writer. The CUDA 13.3.33 dedicated RTX 5090 5-sample startup-only revalidation changed the route and improved mean setup wall to `0.954257x` plus mean token-weight init to `0.913903x`, but kept the profile rejected because token-weight init was unstable with median `1.021956x` and max `1.095803x`.
  - 2026-06-25 reran the same startup-only profile after confirming the RTX 5090 was visible unsandboxed with CUDA UMD 13.3 and no compute processes. The route still changed, but total setup wall improved only to `0.984342x` and `setup.token_weight_init.total_ms` regressed to `1.009464x` mean, `1.001840x` median, and `1.048989x` max, so `NFN_NATIVE_GPT_TOKEN_WEIGHT_BF16_PATTERN_INIT=1` remains diagnostic-only.
...
- [x] Keep the LayerNorm affine row chunk at 128 after the 64-row recheck. `NFN_SM120_NATIVE_CANDIDATE_PROFILE=layernorm_affine_row_chunk_64` proves the smaller route but misses hot-stage gates (`1.004276x` MLP projection, `1.000446x` LM-head backward), so this is a rejected diagnostic rather than a default.
...
- [ ] Close the remaining SM120 parity gap with new kernel work, not more default-switch promotion. The next high-value implementation target is the LM-head classifier/backward contract reported in runtime JSON: fuse or otherwise co-schedule row-chunked BF16 logits, public-vocab CE/dlogits, dHidden, and dWeight so the default path is closer to llm.kittens' full-resident fused classifier without triggering the full-logit memory cliff.
  ...
  - 2026-06-23 added and rejected the narrower `NFN_SM120_NATIVE_CANDIDATE_PROFILE=layernorm_affine_row_chunk_64` profile. The 5-step, 3-sample dedicated RTX 5090 stage-timed gate changed `block_state_layout.layer_norm_backward_affine_row_chunk_size` from 128 to 64 and improved train-loop wall to `0.998045x`, but failed hot-stage gates with `1.004276x` MLP projection and `1.000446x` LM-head backward. Keep the 128-row default unless a future kernel change removes the adjacent-stage regressions.
  ...
  - 2026-06-25 promoted `linear_bias_threads_512` after rerunning it on the
    display-disabled RTX 5090 with CUDA 13.3.33. The same-script 3-step,
    2-sample stage-timed gate proved the route changed
    (`block_state_layout.linear_backward_bias_threads_per_block: 256 -> 512`)
    and measured `train_loop_wall_ms_per_step=0.984417x`,
    `train_loop_cuda_event_steady_state_wall_ms_per_step=0.999644x`,
    `train_tokens_per_second=1.015834x`,
    `stage.block_backward.total_ms=0.971708x`,
    `stage.block_backward.mlp_fc.dweight_bias.total_ms=0.859800x`, and
    `stage.block_backward.mlp_proj.dweight_bias.total_ms=0.930817x`. The
    Tile-CUDA bias reducer now defaults to 512 threads while retaining
