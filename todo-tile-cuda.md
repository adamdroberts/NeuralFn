...
- [ ] Close the remaining measured SM120 throughput gap between the NeuralFn-owned `libnfn_native_train_tile_ops.so` loop and the `llm.kittens` SM120 reference script.
  ...
  - 2026-06-18 promoted cuBLASLt heuristic index 1 for the hot MLP projection dWeight shape `3072,768,65536,N,T`: the dedicated RTX 5090 native-vs-native 5-step, 3-sample confirmation with `NFN_NATIVE_LINEAR_CUBLASLT_HEURISTIC_SHAPE=3072,768,65536,N,T,1` measured `0.998595x` train-loop wall time, `1.001407x` tokens/sec, and `0.998190x` total wall time versus the previous default, with zero compute processes before/after every paired sample.
