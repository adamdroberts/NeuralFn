...
- [x] Live-validate GPT-2 `--train-transformer-lm` memory/runtime behavior at the SM120 default batch shape and compare throughput against `/mnt/disk2/dev/open-source/llm.kittens/train-sm120.sh`.
  ...
  - 2026-06-18 `NFN_TILE_CUDA_EXTRA_NVCC_FLAGS=-DLLMK_SM120_ATOMIC_DQ` is not currently a direct Tile ops benchmark candidate: the compile fails because llm.kittens switches q-gradient storage to float and removes the packed-gradient entrypoint used by NeuralFn's packed-QKV wrapper. Testing atomic-dQ requires a dedicated wrapper path that supplies float dQ scratch and converts/re-packs q-gradients before the packed QKV dWeight handoff.
