...
- [x] Live-validate GPT-2 `--train-transformer-lm` memory/runtime behavior at the SM120 default batch shape and compare throughput against `/mnt/disk2/dev/open-source/llm.kittens/train-sm120.sh`.
  ...
  - 2026-06-19 keep the current LM-head hot-shape bisections rejected on CUDA 13.3: `NFN_NATIVE_LINEAR_TK_FORWARD_ENABLE_SHAPE=50304,8192,768,T,N` activated the LM-head logits TK route but measured `1.005595x` train-loop wall time and `0.994454x` tokens/sec versus default; `NFN_NATIVE_LINEAR_TK_DINPUT=1` measured `1.053838x` train-loop wall time and `0.949018x` tokens/sec; `NFN_NATIVE_LINEAR_CUBLASLT_HEURISTIC_SHAPE=768,50304,8192,N,T,0` measured `1.003965x` train-loop wall time and `0.996050x` tokens/sec. No default changed for those routes.
...
- [ ] Close the remaining measured SM120 throughput gap between the NeuralFn-owned `libnfn_native_train_tile_ops.so` loop and the `llm.kittens` SM120 reference script.
  ...
  - 2026-06-18 rejected promoting `CUDA_MODULE_LOADING=LAZY` from the wrapper environment: the dedicated RTX 5090 3-step, 2-sample same-script candidate measured `0.997976x` train-loop wall time and `1.002029x` tokens/sec, but total wall time stayed neutral at `0.999180x` and setup regressed to `1.017474x` with token init at `1.029699x`. Keep module loading unset by default.
  ...
  - 2026-06-18 rejected re-enabling TK BF16 forward for the padded LM-head logits shape with `NFN_NATIVE_LINEAR_TK_FORWARD_ENABLE_SHAPE=50304,8192,768,T,N`: the dedicated RTX 5090 native-vs-native 5-step, 3-sample run measured `1.007997x` train-loop wall time, `0.992076x` tokens/sec, and `1.006142x` total wall time versus the current GEMMEx fallback.
...
- [x] Rebuilt and rechecked dense GPT native after the CUDA 13.3 reinstall: `bash tools/build_native_gpt_cli.sh`, `python -m pytest tests/test_native_gpt2.py -q`, `python tools/check_native_no_torch_deps.py`, and `git diff --check` passed.
- [x] Refreshed the canonical NeuralFn-vs-llm.kittens parity baseline with selected-GPU locking and idle checks: `NFN_SM120_PARITY_STEPS=5 NFN_SM120_PARITY_SAMPLES=2 NFN_SM120_PARITY_WARMUP=0 NFN_SM120_PARITY_STAGE_TIMING=1 ... bash tools/bench_native_gpt_sm120_parity.sh` measured llm.kittens at `2489.190 ms/step` and NeuralFn at `2570.610 ms/step`, or `1.032743x` train-loop wall time and `0.967645x` tokens/sec versus `/mnt/disk2/dev/open-source/llm.kittens/train-sm120.sh`.
