...
- [x] Add same-script benchmark profiles for long-run prewarm policy changes.
  ...
  strategy value to change. The legacy deferred profile gates
...
- [x] Keep the CUDA 13.3 train-loss scalar-copy cleanup and MLP activation-store memory probes out of the critical-path candidate list. The loss-copy default now relies on blocking D2H `cudaMemcpy` ordering and keeps `NFN_NATIVE_GPT_LM_HEAD_LOSS_COPY_SYNC=1` only for same-binary diagnostics; a dedicated RTX 5090 5-step, 3-sample run with `--train-loss-every-steps 1` measured the no-sync default as neutral/noise-level (`1.000906x` train-loop wall, `0.999097x` tokens/sec). Reducing stored MLP activation blocks from 12 to 11 (`NFN_NATIVE_GPT_STORE_MLP_BLOCKS=11`) did not improve setup and regressed train-loop wall time to `1.024478x` with `0.976120x` tokens/sec because recompute increased BF16 pack/cache churn. A 2026-06-28 3-step, 1-sample stage-timed `store_mlp_blocks6_tail` gate compared six stored MLP activation blocks at head versus tail placement; tail placement improved `setup_wall_ms` to `0.957064x`, but regressed train-loop wall to `1.010155x`, steady-state CUDA-event timing to `1.010887x`, and candidate-over-llm.kittens train-loop wall to `1.197974x`. The matching `store_packed_attention_blocks6_tail` gate improved setup to `0.961199x`, but regressed train-loop wall to `1.008900x`, block recompute to `1.201126x`, and candidate-over-llm.kittens train-loop wall to `1.068327x`. Do not promote these as parity fixes.
...
- [ ] Wire dense GPT native training to real packed NVFP4 activation buffers and projection/attention FP4 GEMM routes. The current compiled dense GPT trainer preserves `--tile-cuda-activation-dtype nvfp4` as intent only and reports `native_activation_packing_active: false`; `--require-native-nvfp4-activation-packing` now fails fast until this native storage path is connected to the trainer.
  ...
  - [x] Add the same-script SM120 `nvfp4_qkv_dweight` candidate profile for the existing partial packed-LN1 sidecar. The profile forces `--tile-cuda-activation-dtype nvfp4`, sets `NFN_NATIVE_GPT_NVFP4_QKV_DWEIGHT=1`, requires `block_backward_nvfp4_qkv_dweight_pack_count` / `block_backward_nvfp4_qkv_dweight_count` and `block_backward_qkv_dweight_strategy` to change, and gates `stage.block_backward.qkv.dweight_bias.total_ms` against the BF16 default before any promotion. A 2026-06-28 dedicated RTX 5090 3-step, 1-sample stage-timed run proved 288 NVFP4 pack/dWeight calls but kept the profile rejected at `2.699758x` train-loop wall, `2.692837x` steady-state CUDA-event timing, `4.335894x` block backward, and `42.421293x` QKV dWeight+bias. This is still a diagnostic partial route, not the full activation-packing completion item.
...
- [ ] Close the remaining SM120 parity gap with new kernel work, not more default-switch promotion. The next high-value implementation target is the LM-head classifier/backward contract reported in runtime JSON: fuse or otherwise co-schedule row-chunked BF16 logits, public-vocab CE/dlogits, dHidden, and dWeight so the default path is closer to llm.kittens' full-resident fused classifier without triggering the full-logit memory cliff.
  ...
  - 2026-06-28 checked adjacent three-chunk LM-head row sizes around the current 28672-row default on the dedicated RTX 5090. `lm_head_row_chunk_24576` kept the same graph replay count and reduced `lm_head_bf16_logit_bytes` to `0.857143x`, total uint16 arena bytes to `0.981486x`, and setup wall to `0.958578x`, but rejected default promotion at `1.002694x` train-loop wall, `1.002341x` steady-state CUDA-event timing, `1.005528x` LM-head backward, and `0.997312x` tokens/sec. `lm_head_row_chunk_30720` increased logit bytes to `1.071429x` and regressed train-loop wall to `1.026742x`, LM-head backward to `1.106116x`, and tokens/sec to `0.973958x`. Keep 28672 rows as the measured default; the remaining gap needs a real LM-head/block kernel improvement rather than more neighboring row-chunk tuning.
  ...
  - 2026-06-30 refreshed the bounded same-script parity gate with five warmup
    pairs and two measured five-step samples:
    `NFN_SM120_NATIVE_DEFAULT_LONG_RUN_DEFER_PREWARM=0
    NFN_SM120_NATIVE_LONG_RUN_DEFER_PREWARM_MIN_WARMUP=0
    NFN_SM120_NATIVE_LONG_RUN_DEFER_PREWARM_MIN_STEPS=0
    NFN_SM120_NATIVE_STEPS=5 NFN_SM120_NATIVE_WARMUP=5
    NFN_SM120_NATIVE_SAMPLES=2
    NFN_SM120_NATIVE_JSON_OUT=/tmp/nfn_sm120_parity_current_20260630.json
    tools/bench_native_gpt_sm120_parity.sh`. The run selected the display-disabled
    RTX 5090, acquired the GPU lock, observed zero external compute processes,
    and passed the native runtime contract (`graph_editor_tensor_flow=false`,
    `torch_required=false`, optimized-kernel contract true, and
    `train_loss_host_d2h_count=0`). NeuralFn measured `2466.050 ms/step`
    against llm.kittens `2480.902 ms/step`, or `0.994015x` train-loop wall,
    `0.995455x` steady-state CUDA-event wall, and `1.006521x` tokens/sec. The
    current dense GPT path therefore passes this bounded parity check; remaining
    work should stay focused on broader native-family trainer coverage and real
    LM-head/block kernel fusion rather than graph-editor, Torch, or external-load
    hypotheses.
