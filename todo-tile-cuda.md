...
- [x] Make the dense native GPT optimizer route a hard optimized-kernel startup contract. Full transformer training now requires the many-tensor AdamW, BF16-primary/shadow AdamW, BF16-gradient AdamW, many-buffer sumsq, and device clip-scale Tile-CUDA symbols before setup proceeds; runtime JSON reports `optimized_optimizer_contract_loaded` and `optimized_optimizer_contract_error` so same-script gates can reject scalar or per-buffer optimizer fallbacks.
