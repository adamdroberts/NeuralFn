...
fallback unless that macro is set. In the full dense GPT trainer, block
forward/recompute projections consume BF16-primary block weights through
`nfn_native_tile_linear_weight_bf16_float32`,
`nfn_native_tile_linear_weight_bf16_output_float32`, and
`nfn_native_tile_linear_bf16_input_weight_bf16_float32`; block dInput GEMMs call
`nfn_native_tile_linear_backward_input_weight_bf16_float32`. LN1 packed-QKV
forward uses `nfn_native_tile_layer_norm_with_stats_bf16_out_float32` plus
`nfn_native_tile_linear_bf16_input_weight_bf16_output_float32` and reports
`qkv_forward_ln1_bf16_enabled: true`. Transformer block dWeight+bias
accumulation uses
`nfn_native_tile_linear_backward_weight_bias_accumulate_bf16_float32`,
`nfn_native_tile_linear_backward_weight_bias_accumulate_bf16_bits_float32`, or
the default QKV BF16/BF16 route
`nfn_native_tile_linear_backward_weight_bias_accumulate_bf16_bits_bf16_bits_float32`.
Tied LM-head logits and dHidden default to the BF16 TK/classifier path, and
native JSON reports `linear_backend_strategy:
...
Native GPT trained checkpoint export uses the raw Tile ABI
...
Native GPT-2 handoff helpers are exported from `neuralfn.native_gpt2` and top-level `neuralfn`: `NativeGpt2RunConfig`, `NativeGpt2RunnerStatus`, `NativeGpt2CheckpointInfo`, `build_native_gpt2_run_config()`, `build_native_gpt2_compiled_cli_run_config()`, `native_gpt2_runner_status()`, `resolve_native_gpt2_cli()`, `resolve_native_gpt2_launcher()`, `resolve_native_gpt2_token_shards()`, `write_native_gpt2_run_config()`, `read_native_gpt2_checkpoint_info()`, `is_native_gpt2_checkpoint()`, `latest_native_gpt2_checkpoint()`, `native_gpt2_parameter_count()`, and `run_native_gpt2()`. They materialize/resolve uint16 train and validation token shards and produce compiled native commands so the GPT compatibility script can bypass graph-editor node data flow. `build_native_gpt2_compiled_cli_run_config()` is a compatibility path for cached shards: it passes a dataset alias/path to the compiled GPT CLI, leaves shard metadata validation to C++, and canonicalizes dense GPT selectors to `model_family: "gpt"`. The C++ resolver accepts NeuralFn `fineweb_*` shards, llm.kittens `TinyStories_train.bin` / `TinyStories_val.bin`, `--tinystories` through `/mnt/disk2/dev/open-source/llm.kittens/dev/data/tinystories` or `NFN_LLM_KITTENS_TINYSTORIES_DIR`, and direct train-bin paths with inferred validation siblings. `NativeGpt2RunConfig.kernel_backend` is strictly `"tile-cuda"` and `NativeGpt2RunConfig.train_transformer_lm` defaults to `True`; the removed `"llm-kittens"` backend is rejected rather than routed to the external `train_gpt2cu` bridge. `neuralfn.config.SHIPPED_GPT_TEMPLATE_PRESETS` and the lazy top-level `neuralfn.SHIPPED_GPT_TEMPLATE_PRESETS` export are the canonical template-name catalog for native selector coverage. `NativeGpt2RunConfig.template_name` and `NativeGpt2RunConfig.graph_file` forward every shipped GPT template name or custom graph selection to the compiled native CLI without importing Torch; the compiled C++ plan JSON reports the synchronized `shipped_template_catalog`, `shipped_template_catalog_count`, `template_known`, `architecture_source`, `architecture_contract`, and `model_family_context_policy` fields. The seven reviewed dense GPT-2 profiles (`gpt2`, `gpt2_megakernel`, `gpt2_moa`, `gpt2_zloss`, `gpt2_softcap`, `gpt2_qknorm`, and `gpt2_stable`) map to the implemented native loop and dry-run/plan JSON reports `native-transformer-lm-ready` with `training_step_plan.status: "ready"`, while `gpt2_moa` resolves `NativeGpt2RunConfig.activation` to `moa` automatically. Structurally different shipped templates and custom graphs report `selected-graph-native-trainer-missing` until their C++ Tile trainer plans exist; unknown template names report `unknown-template`. `NativeGpt2RunConfig.smoke_transformer_lm_step` forwards `--smoke-transformer-lm-step`, which samples cached uint16 tokens, preserves range-checked GPT-2 token IDs, and runs token/position embeddings, one tiny transformer block, final LayerNorm, tied LM head, CE forward/backward, transformer backward, embedding backward, and AdamW for 16 parameter buffers through raw Tile kernels without Torch. `NativeGpt2RunConfig.train_transformer_lm` forwards `--train-transformer-lm`, which runs that transformer-LM path as a full-vocab real-dim 12-layer multi-step compiled loop over cached shards with periodic validation records in `validation.losses`, token/position embeddings, transformer blocks, final norm, a row-chunked tied LM-head/CE workspace, transformer backward, embedding backward, device-side global norm gradient clipping, and AdamW parameter updates without Python/Torch; block allocation, initialization, gradient zeroing, gradient clipping, AdamW updates, checkpoint export, activation tape, forward block execution, and backward block execution are driven from per-block C++ state/tape vectors with `activation_tape_strategy: "scratch-recompute"`. The real training loop treats `train_batch_tokens` as the effective optimizer-step batch: it derives `grad_accum_steps` from `batch_size * seq_len`, streams that many cached-shard CUDA Tile microbatches, averages gradients in device accumulation buffers through `nfn_native_tile_gradient_accumulate_float32`, then clips and runs AdamW once. Its JSON reports `microbatch_tokens`, `requested_train_batch_tokens`, `grad_accum_steps`, `effective_train_batch_tokens`, `train_microbatches_completed`, and `gradient_accumulation_strategy`. The real training loop keeps token/target batches as uint16, stages them through pinned host memory, enqueues `cudaMemcpyAsync`, widens to int64 on device through `nfn_native_tile_uint16_to_int64`, and initializes the tied token embedding/LM-head weight directly on device through `nfn_native_tile_init_gpt2_token_weight_float32`; its JSON reports `token_id_upload_strategy: "uint16-pinned-async-h2d-device-widen"`, `token_weight_init_strategy: "device-tile-deterministic"`, and `token_weight_host_materialization: false`. `NativeGpt2RunConfig.lm_head_row_chunk_size` defaults to 28672 and forwards `--lm-head-row-chunk-size`; the C++ loop reduces CE loss partials on device with `nfn_native_tile_sum_partials_float32` before a single host loss copy, and tied LM-head dWeight chunks accumulate directly into `accum_grad_token_weight` with `nfn_native_tile_linear_backward_weight_accumulate_float32` instead of using a full-vocab scratch gradient buffer per chunk or per microbatch. Its JSON reports `trained_layers: 12`, `target_layers: 12`, `block_state_layout` with the block-vector loop flags, `vocab: 50257`, `lm_head_row_chunk_size`, `lm_head_row_chunk_count`, `loss_partial_count`, `logit_workspace_elements`, `gradient_partial_count`, `gradient_clip_norm`, `sample_gradient_clip_scale`, and final 12-layer trained checkpoint metadata when steps complete. `NativeGpt2RunConfig.checkpoint_metadata_smoke` forwards `--checkpoint-metadata-smoke`, which writes a sparse version-5 bf16 native GPT-2 checkpoint-format file plus `DONE_########` marker for the requested `--num-layers` target shape so `read_native_gpt2_checkpoint_info()` and native inference metadata can validate NeuralFn-owned artifacts without CUDA. Successful `--train-transformer-lm` runs write a final 12-layer trained-weight native checkpoint plus `DONE_########` marker. Native checkpoint helpers read `model_########.bin` headers plus `DONE_########` markers without importing Torch, so `nfn infer --checkpoint PATH --native-info` can identify native artifacts instead of sending them to the graph-backed `.pt` loader. The compiled GPT CLI also supports `--native-info --native-checkpoint PATH` and `--inspect-checkpoint PATH` for C++ JSON inspection before CUDA, token-shard resolution, Torch, Python dataset setup, or graph-node execution. Native `.bin` prompt generation is available through `nfn_gpt_native_train --sample-checkpoint PATH --prompt-tokens IDS` and the matching `nfn infer` wrappers. Build the SDK binding with `bash tools/build_native_gpt2_binding.sh`, launcher with `bash tools/build_native_gpt2_launcher.sh`, no-Python cached-shard CLI with `bash tools/build_native_gpt_cli.sh`, and unified native frontend with `bash tools/build_native_train_cli.sh`; the compiled GPT CLI links `token_shards.cpp` for shared no-Torch shard validation. `tools/install_native_gpt2_commands.sh` links stable commands including `nfn-gpt-native` and `nfn-native-train`, and `nfn-native-train --base-model gpt --dataset-alias PATH_OR_ALIAS` bypasses Python entirely when cached shards already exist. `nfn-native-train --list-models --json` reports native coverage; dense GPT and NanoGPT selectors use the implemented shared Tile-CUDA trainer, while LLaMA/GPT-2 evo/JEPA/semantic-MoE/DeepSeek variants report their current covered, missing, or preflight-only native status. The CLI wrapper defaults to `--native-cuda-runner compiled-cli`, while `run_native_gpt2(..., runner="auto")` still prefers an in-process C++ binding module (`neuralfn_native_gpt2` or `neuralfn._native_gpt2`), then the compiled no-Python CLI, then the compiled launcher, then direct subprocess execution. `runner="binding"` requires the binding, `runner="compiled-cli"` requires the no-Python CLI, and `runner="launcher"` requires the launcher. Top-level `neuralfn` exports are lazy; importing `neuralfn.native_gpt2` should not import Torch. NeuralFn no longer exposes a `.[torch]` packaging extra; install `.[tile-cuda]` for native CUDA Tile tooling and manage PyTorch separately only when running legacy graph-backed Torch code.

The current strict dense-GPT SDK contract adds `lr_schedule`,
`lr_schedule_total_steps`, `train_seed`, `resume_from_checkpoint`, and
`graph_fingerprint` plus `graph_preflight_proof` to `NativeGptRunConfig` /
`NativeGpt2RunConfig` and their
builders; `graph_file` remains the source path and `train_seed` is distinct
from generation `seed`. The builders accept optional `final_lr_fraction`; an
explicit fraction takes precedence over the value derived from `min_lr`, and
schedule, final-LR, and train-loss aliases normalize before native quality
defaults. `compiled_cli_argv()` forwards
all of them, including an explicitly selected target for alias-only configs.
The legacy llm.kittens short argv cannot express a non-default schedule or the
strict continuation/graph fields, so `argv()`/`command()` raise and `to_dict()`
returns empty legacy `argv`/`command` plus `legacy_command_error` and
`prefer_compiled_cli` while retaining the compiled-CLI and Tile-launcher forms.
Dataset-alias and strict Tile binding
configs prefer the compiled argv. A hand-built config with native-only fields,
no dataset alias, and a legacy short executable rejects instead of silently
changing targets; use the compiled builder or supply the dataset alias and
compiled trainer. Hand-built non-Tile configs also reject native-only fields.
For explicit `gpt2_diff` configs, `graph_file`, `graph_fingerprint`, and
`graph_preflight_proof` are an exact triplet. The digest must be 64 lowercase
hexadecimal characters and neither digest nor proof path may have surrounding
whitespace. All three may be absent only for selector/catalog command
construction; normal differential training rejects an unbound selector before
Tile/CUDA. Obtain the proof from trusted
`plan_native_graph_training(..., materialize=True)` output. Its unkeyed digest
protects local handoff integrity; it is not authenticity for caller-supplied
graph/proof pairs.

Low-level packed `gpt2_diff` uses
`neuralfn.native_gpt2_diff.training_checkpoint` version 2 while its unchanged
five binary formats retain `checkpoint_kind=trained_dense_v5_plus_diff_v1`.
Continuation preflight finishes before Tile/CUDA/H2D and binds the source,
DONE-gated binary state, resolver-ordered training-shard contents,
counters/sampler, seed, accumulation, optimizer/LR horizon, LM-head chunk,
effective BF16 routes, and a canonical numerics profile of supported effective routes;
validation shards are excluded. The learned trainer requires the learned-lambda
ABI. The legacy fixed-lambda ABI remains outside this contract with
rounded-output/non-layer-local backward debt. Migration/resident differential
execution remains blocked.

Native GPT token-weight initialization defaults to the vector4 CUDA Tile path for GPT-sized tables and reports `token_weight_vector4_init_enabled`; set `NFN_NATIVE_GPT_TOKEN_WEIGHT_VECTOR4_INIT=0` only for paired comparison against the previous fast-int32 Tile path. The fused BF16 shadow writer keeps the conversion-based vector4 route by default; set `NFN_NATIVE_GPT_TOKEN_WEIGHT_BF16_PATTERN_INIT=1` only for paired startup comparison against the rejected precomputed-pattern shadow writer.
...
`NativeGpt2RunConfig.kernel_backend` is strict: `tile-cuda` is the only
NeuralFn-owned 12-layer trainer path, and the removed `llm-kittens` backend is
rejected. The `tile-cuda` path may print a plan, verify `tile_ops_lib`, or run
...
or pass `--smoke-tile-ops --tile-ops-lib PATH` to `nfn_gpt_native_train`.
It loads CUDA runtime, launches `nfn_native_tile_fill_float32`, copies the
tiny device buffer back, and emits JSON. The only accepted backend name is
`tile-cuda`.
...
`linear_cublaslt_grouped_matmul_probe_status`; on the current CUDA 13.3 RTX
5090 route that status is `15`, so do not route training through grouped
cuBLASLt matmul until a later probe reports supported.

`bash tools/build_native_missing_trainers.sh` builds per-family native targets at their current covered, diagnostic, or blocked status; the script name does not imply that every emitted target is incomplete. GPT-2 evo is a C++ native preflight target; `nfn_gpt2_evo_native_train --print-plan --eval-every-steps 1000 --tile-cuda-activation-dtype nvfp4` reports the dense GPT-2 shape, `adamw` optimizer profile, validation cadence, NVFP4 activation intent, evo-layer index/cadence/population metadata, device-side mutation/selection/adoption kernels, and dense GPT native delegation for forward candidate evaluation without Python/Torch. NanoGPT's separate per-family C++ binary is a token-LM diagnostic; full transformer training uses the implemented shared dense selector. `nfn_nanogpt_native_train --print-plan --require-token-shards --sample-token-batch` reports the resolved shard metadata, first native token/target batch, contiguous parameter/gradient/AdamW-state buffers, AdamW decay/no-decay groups, forward/backward/optimizer `training_step_plan`, tied LM head backward coverage through the linear ABI, and kernels exposed through the native ABI without Python/Torch. `nfn_nanogpt_native_train --check-tile-ops --tile-ops-lib PATH` verifies the diagnostic binary's compiled binding to the raw trainer ABI, `--smoke-tile-ops --tile-ops-lib PATH` verifies an actual fill-kernel launch/copyback through dynamically loaded CUDA runtime, `--smoke-optimizer-step --tile-ops-lib PATH` verifies registered-buffer AdamW iteration over the NanoGPT parameter layout, `--smoke-training-loop-step --tile-ops-lib PATH` verifies native optimizer-loop zero/clip/scale/update mechanics over that same registered layout, `--smoke-lm-step --tile-ops-lib PATH` verifies a tiny tied-embedding LM forward/backward/update path over raw kernels, `--smoke-token-train-step --tile-ops-lib PATH --dataset-alias PATH_OR_ALIAS` verifies the same tied-LM kernel path over a real sampled native uint16 token/target batch, `--train-token-lm --tile-ops-lib PATH --dataset-alias PATH_OR_ALIAS --max-steps N` runs that tied-LM path as a real multi-step native training loop with periodic validation losses over validation shards, `--smoke-fused-qkv-attention-step --tile-ops-lib PATH` verifies fused `attn.qkv.weight` projection, QKV split, SDPA forward/backward, QKV gradient merge, fused qkv weight backward, output projection backward, and AdamW updates over raw kernels, `--smoke-transformer-block-step --tile-ops-lib PATH` verifies a composed LayerNorm/fused-QKV attention/residual/MLP/backward/AdamW block path over raw kernels, `--smoke-mlp-step --tile-ops-lib PATH` verifies MLP projection/GELU forward/backward/update over raw kernels, and `--smoke-attention-step --tile-ops-lib PATH` verifies the separate-Q/K/V attention comparison path. The other targets report their own current covered or missing CUDA Tile contracts for LLaMA, MixLLaMA, JEPA, semantic-router MoE, and DeepSeek-V4; use `native_train_model_registry()` rather than the helper script name as the readiness source of truth.
