...
Canonical dense GPT handoff helpers are exported from `neuralfn.native_gpt` and top-level `neuralfn`: `NativeGptRunConfig`, `NativeGptRunnerStatus`, `NativeGptCheckpointInfo`, `build_native_gpt_run_config()`, `build_native_gpt_compiled_cli_run_config()`, `native_gpt_runner_status()`, `resolve_native_gpt_cli()`, `write_native_gpt_run_config()`, `read_native_gpt_checkpoint_info()`, and `run_native_gpt()`. Prefer these names for new SDK code. `gpt`, `gpt2`, and `gpt3` route to the same dense GPT CUDA Tile C++ trainer; `gpt3` only defaults to a 2048-token context when no template, graph, or explicit sequence length is provided. The older `neuralfn.native_gpt2` helpers remain compatibility wrappers around the same implementation and checkpoint format.
