...
Native GPT token-weight initialization defaults to the int32 Tile-index fast path when the table fits in int32 and reports `token_weight_fast_int32_init_enabled`; set `NFN_NATIVE_GPT_TOKEN_WEIGHT_FAST_INT32_INIT=0` only for paired comparison against the older int64-index Tile path.
