...
- Current `nfn_gpt_native_train --sample-checkpoint PATH --prompt-tokens IDS --max-new-tokens N` no longer returns the old pending-plan JSON: it executes the full checkpoint stack once per generated token through CUDA Tile kernels and returns up to `N` IDs in `generated_tokens`. Keep text-prompt native tokenization marked pending until that path is implemented.
