...
def test_native_gpt2_cpp_cli_builds_and_uses_sm120_defaults(tmp_path: Path) -> None:
    ...
    assert tile_payload["attention_forward_strategy"] == "tk-sm120-bf16-flashattention-bridge"
    ...
    assert tile_payload["packed_qkv_attention_enabled"] is False
    assert tile_payload["packed_qkv_attention_bf16_elements"] == 0
    assert tile_payload["packed_qkv_attention_bf16_bytes"] == 0
    assert tile_payload["packed_qkv_float_attention_tape_elided"] is False
    assert tile_payload["packed_qkv_float_attention_tape_elements_elided"] == 0
    assert tile_payload["packed_qkv_float_attention_tape_bytes_elided"] == 0
    assert tile_payload["qkv_forward_layout_strategy"] == "fused-split-to-heads"
    assert tile_payload["qkv_forward_layout_kernel_launches_per_block"] == 1
    assert tile_payload["qkv_forward_layout_legacy_launches_per_block"] == 4
    assert tile_payload["qkv_forward_layout_launches_elided_per_block"] == 3
    assert tile_payload["qkv_bias_layout_strategy"] == "fused-qkv-bias-split-to-heads"
    assert tile_payload["qkv_bias_fused_tk_gemm_enabled"] is False
    ...
    assert tile_payload["qkv_backward_layout_strategy"] == "fused-heads-to-qkv"
    ...
    assert tile_payload["attention_backward_bf16_qkv_grad_handoff_enabled"] is False
    assert tile_payload["attention_backward_direct_bf16_qkv_grad_scratch_enabled"] is False
    assert tile_payload["attention_backward_direct_bf16_qkv_grad_scratch_elements"] == 0
    assert tile_payload["attention_backward_qkv_bridge_strategy"] == "fused-bf16-heads-to-row-qkv"
    assert tile_payload["attention_backward_qkv_bridge_kernel_launches_per_block"] == 1
    ...
    assert tile_payload["attention_packed_output_unpack_strategy"] == "not-packed"
    ...
    assert tile_payload["stored_packed_attention_ln1_bf16_enabled"] is False
    assert tile_payload["stored_packed_attention_ln1_bf16_blocks"] == 0
